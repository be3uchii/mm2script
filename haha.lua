...
local function updateHitboxes()
    for player, data in pairs(hitboxHighlights) do
        if data then
        ...
        if player ~= LocalPlayer and player.Character then
            ...
            if humanoid and humanoidRootPart and humanoid.Health > 0 then
                if not hitboxHighlights[player] then
                    ...
                    hitboxPart.Size = Vector3.new(settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size)
                    hitboxPart.Transparency = 1
                    hitboxPart.CanCollide = false
                    hitboxPart.Anchored = true
                    hitboxPart.CastShadow = false
                    hitboxPart.Parent = humanoidRootPart

                    hitboxHighlights[player] = {highlight = highlight, part = hitboxPart}
                else
                    local data = hitboxHighlights[player]
                    data.highlight.FillColor = settings.Combat.Hitbox.Color
                    data.highlight.FillTransparency = settings.Combat.Hitbox.Transparency
                    data.highlight.OutlineColor = settings.Combat.Hitbox.Color
                    data.highlight.OutlineTransparency = settings.Combat.Hitbox.Transparency
                    data.part.Size = Vector3.new(settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size)
                end
            elseif hitboxHighlights[player] then
                hitboxHighlights[player].highlight:Destroy()
                if hitboxHighlights[player].part then
                    hitboxHighlights[player].part:Destroy()
                end
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if hitboxHighlights[player] then
                    hitboxHighlights[player].part.CFrame = humanoidRootPart.CFrame
                    hitboxHighlights[player].highlight.Adornee = humanoidRootPart
                end
            end
        end
    end)
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        createValueChanger(scrollFrame, "Размер Хитбокса", {2, 3, 4, 5, 6, 7, 8, 10, 12}, 3, function(value)
    ...
    elseif tabName == "Misc" then
        ...
        createValueChanger(scrollFrame, "Значение Скорости", {16, 20, 25, 30, 35, 40, 50, 60, 70}, 1, function(value)
...
game:BindToClose(function()
    ...
    for _, data in pairs(hitboxHighlights) do
        if data then
            if data.highlight then data.highlight:Destroy() end
            if data.part then data.part:Destroy() end
        end
