...
local currentTab = 1
local tabs = {"Combat", "ESP", "Config", "Chat"}
...
local bannedWords = {
    "хуй", "пизда", "ебал", "ебать", "блядь", "блять", "сука", "пидор", "пидр", "гандон", 
    "гондон", "мудак", "мудила", "залупа", "шлюха", "чмо", "даун", "дебил", "долбоеб", 
    "еблан", "ебанутый", "ебнутый", "заебал", "уебок", "хуесос", "хуйня", "пиздец", 
    "fuck", "shit", "bitch", "asshole", "dick", "pussy", "cock", "cunt", "nigger", 
    "nigga", "whore", "slut", "faggot", "retard", "motherfucker", "damn", "bastard"
}

local function containsBannedWords(text)
...
local function refreshChatUI(scrollFrame)
    ...
    chatMessages.Size = UDim2.new(1, 0, 0.7, 0)
    ...
    chatMessages.ScrollBarImageColor3 = Color3.fromRGB(60, 60, 255)
    chatMessages.CanvasSize = UDim2.new(0, 0, 0, 0)
    chatMessages.Selectable = false
    chatMessages.Parent = scrollFrame
    
    local layout = Instance.new("UIListLayout")
    layout.Padding = UDim.new(0, 5)
    layout.Parent = chatMessages
    
    layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        chatMessages.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 5)
    ...
    sendButton.Size = UDim2.new(0.25, -5, 0.8, 0)
    sendButton.Position = UDim2.new(0.75, 0, 0.1, 0)
    ...
    local function addMessage(sender, message)
        ...
        messageFrame.Parent = chatMessages
        
        corner:Clone().Parent = messageFrame
        
        local senderLabel = Instance.new("TextLabel")
        senderLabel.Size = UDim2.new(0.3, 0, 1, 0)
        senderLabel.Position = UDim2.new(0, 5, 0, 0)
        senderLabel.BackgroundTransparency = 1
        senderLabel.Text = sender .. ":"
        senderLabel.TextColor3 = Color3.fromRGB(100, 150, 255)
    ...
    local function sendMessage()
        ...
        if message == "" or #message > 100 then return end
        
        if containsBannedWords(message) then
            addMessage("Система", "Ваше сообщение содержит запрещенные слова!")
            textBox.Text = ""
            return
        end
        
        local currentTime = os.time()
        if currentTime - settings.Chat.LastMessageTime < 5 then
            local remaining = 5 - (currentTime - settings.Chat.LastMessageTime)
            addMessage("Система", string.format("Подождите %d секунд перед отправкой следующего сообщения", remaining))
            return
        end
        
        settings.Chat.LastMessageTime = currentTime
        saveSettings()
        
        ReplicatedStorage:FindFirstChild("SilenceChatEvent") or Instance.new("RemoteEvent", ReplicatedStorage).Name = "SilenceChatEvent"
        ReplicatedStorage.SilenceChatEvent:FireServer(LocalPlayer.Name, message)
        textBox.Text = ""
    end
    ...
    if not connections.ChatEvent then
        connections.ChatEvent = ReplicatedStorage:WaitForChild("SilenceChatEvent").OnClientEvent:Connect(function(sender, message)
...
local function createPasswordGUI()
    ...
    timerLabel.Text = "До генерации: 01:00"
    ...
    generateButton.Position = UDim2.new(0.1, 0, 0, 110)
    ...
    local function updateTimerDisplay()
        ...
        timerLabel.Text = string.format("До генерации: %02d:%02d", minutes, seconds)
    end
    ...
    local function startTimer()
        ...
        timerConnection = RunService.Heartbeat:Connect(function(delta)
            ...
            if timer <= 0 then
                ...
                canGenerate = true
                cooldown = false
                timerLabel.Text = "Можно сгенерировать"
                timerConnection:Disconnect()
            else
                updateTimerDisplay()
            end
    ...
    local function generateNewPassword()
        if cooldown then
            errorLabel.Text = "Подождите " .. math.ceil(timer) .. " секунд"
    ...
    local function checkPassword()
        ...
        if isfile(keyPath) then
            ...
            if success and data and data.password and data.expireTime then
                if password == data.password and os.time() < data.expireTime then
                    playSound()
                    passwordGui:Destroy()
                    return true
                end
        ...
        if password == generatedPassword and generatedPassword ~= "" then
            ...
            local tween = TweenService:Create(mainFrame, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -125, -1, 0)})
            tween:Play()
...
local function hasValidKey()
    ...
    if not success or not data or not data.password or not data.expireTime then
        return false
    end
