...
for i = 1, maxParticles do
    ...
    particles[i] = {frame = particle, velocity = Vector2.new(math.random(-2, 2) / 30, math.random(4, 8) / 25), alpha = math.random(40, 70) / 100}
end
...
local function updateHitboxes()
    ...
    connections.HitboxCheck = RunService.Heartbeat:Connect(function()
        if tick() - (connections.HitboxLastCheck or 0) < 1.5 then return end
...
local function updateESP()
    ...
    connections.ESPCheck = RunService.Heartbeat:Connect(function()
        if tick() - (connections.ESPLastCheck or 0) < 1.5 then return end
