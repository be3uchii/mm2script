...
local menuWidth, menuHeight = 280, 180 -- Компактнее для оптимизации
local menuX, menuY = (viewportSize.X - menuWidth) / 2, (viewportSize.Y - menuHeight) / 2
local isMinimized = false
local isVisible = true
local currentTab = 1
local tabs = {"Combat", "ESP", "Misc"}
local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut) -- Быстрее, но плавно
...
mainFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 25) -- Темный фон
...
uiStroke.Color = Color3.fromRGB(70, 70, 255) -- Неоновый голубой
...
local particles = {}
local maxParticles = 8 -- Меньше частиц для оптимизации
for i = 1, maxParticles do
    local size = math.random(1, 3)
    local particle = Instance.new("Frame")
    particle.Size = UDim2.new(0, size, 0, size)
    particle.BackgroundColor3 = Color3.fromRGB(90, 90, 255)
    ...
    particles[i] = {frame = particle, velocity = Vector2.new(math.random(-2, 2) / 30, math.random(4, 8) / 25), alpha = math.random(30, 60) / 100}
end
...
local function updateParticles()
    ...
    if tick() - lastParticleUpdate < 0.1 then -- 10 FPS для частиц
        return
    end
    ...
    for _, particle in ipairs(particles) do
        ...
        local newAlpha = particle.alpha + math.sin(tick() * 1.2 + newX * 8) * 0.08 -- Менее заметные изменения
        newAlpha = math.clamp(newAlpha, 0.3, 0.5)
        if newY > 1.5 or newX < -0.1 or newX > 1.1 then
            newX = math.random()
            newY = -0.1
            particle.velocity = Vector2.new(math.random(-2, 2) / 30, math.random(4, 8) / 25)
...
local function createToggle(parent, text, defaultValue, callback)
    ...
    toggle.MouseButton1Click:Connect(function()
        ...
        toggle.Text = defaultValue and "ВКЛ" or "ВЫКЛ"
        local targetColor = defaultValue and Color3.fromRGB(0, 230, 0) or Color3.fromRGB(230, 0, 0)
    ...
    toggle.MouseLeave:Connect(function()
        ...
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(70, 70, 255)}):Play()
    end)
...
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    prevButton.MouseEnter:Connect(function()
        TweenService:Create(prevButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(45, 45, 55), Size = UDim2.new(0.105, 0, 0.82, 0)}):Play()
    end)
    ...
    nextButton.MouseEnter:Connect(function()
        TweenService:Create(nextButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(45, 45, 55), Size = UDim2.new(0.105, 0, 0.82, 0)}):Play()
    end)
...
local function createColorButton(parent, text, defaultColor, callback)
    ...
    colorButton.MouseEnter:Connect(function()
        ...
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(100, 100, 255), Thickness = 1.2}):Play()
    end)
    
    colorButton.MouseLeave:Connect(function()
        TweenService:Create(colorButton, tweenInfo, {Size = UDim2.new(0.3, 0, 0.8, 0)}):Play()
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(70, 70, 255), Thickness = 1}):Play()
    end)
...
local function updateHitboxes()
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if humanoidRootPart then
                    ...
                    humanoidRootPart.CastShadow = false -- Отключение теней
                end
            end
        end
    ...
    connections.HitboxCheck = RunService.Heartbeat:Connect(function()
        if tick() - (connections.HitboxLastCheck or 0) < 3 then return end
...
local function updateESP()
    ...
    connections.ESPCheck = RunService.Heartbeat:Connect(function()
        if tick() - (connections.ESPLastCheck or 0) < 3 then return end
    ...
    if LocalPlayer.Character then
        for _, part in pairs(LocalPlayer.Character:GetDescendants()) do
            if part:IsA("BasePart") then
                ...
                part.CastShadow = false -- Отключение теней
            end
        end
    end
end
...
for i, tabName in ipairs(tabs) do
    ...
    tabButton.BackgroundColor3 = i == currentTab and Color3.fromRGB(70, 70, 90) or Color3.fromRGB(35, 35, 45)
    ...
    tabStroke.Color = i == currentTab and Color3.fromRGB(70, 70, 255) or Color3.fromRGB(50, 50, 70)
    ...
    contentStroke.Color = Color3.fromRGB(70, 70, 255)
    ...
    scrollFrame.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 255)
    ...
    if tabName == "Combat" then
        ...
        createValueChanger(scrollFrame, "Размер Хитбокса", {1, 2, 3, 4, 5, 6}, settings.Combat.Hitbox.Size, function(value)
    ...
    tabButton.MouseButton1Click:Connect(function()
        ...
        for j, btn in ipairs(tabButtons) do
            ...
            btn.BackgroundColor3 = isActive and Color3.fromRGB(70, 70, 90) or Color3.fromRGB(35, 35, 45)
            local stroke = btn:FindFirstChildOfClass("UIStroke")
            if stroke then
                stroke.Color = isActive and Color3.fromRGB(70, 70, 255) or Color3.fromRGB(50, 50, 70)
    ...
    tabButton.MouseLeave:Connect(function()
        if i ~= currentTab then
            TweenService:Create(tabButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(35, 35, 45)}):Play()
        end
    end)
end
...
dragArea.MouseButton1Click:Connect(function()
    ...
    local targetTransparency = isMinimized and 0.2 or 0.1
    ...
    TweenService:Create(dragArea, tweenInfo, {TextTransparency = isMinimized and 0.2 or 0.15, TextSize = isMinimized and 12 or 14}):Play()
end)
...
RunService:BindToRenderStep("MenuUpdate", Enum.RenderPriority.Input.Value + 1, function()
    ...
    for i, btn in ipairs(tabButtons) do
        local targetColor = i == currentTab and Color3.fromRGB(70, 70, 90) or Color3.fromRGB(35, 35, 45)
...
game:BindToClose(function()
    ...
    Lighting.GlobalShadows = true -- Восстановление теней
    screenGui:Destroy()
end)