...
for i = 1, maxParticles do
    ...
    particle.BackgroundColor3 = Color3.fromRGB(80, 150, 255)
    particle.BackgroundTransparency = math.random(40, 70) / 100
    particle.Parent = particleCanvas
    local particleCorner = Instance.new("UICorner")
    particleCorner.CornerRadius = UDim.new(1, 0)
    particleCorner.Parent = particle
    particles[i-1] = {frame = particle, velocity = Vector2.new(math.random(-2, 2) / 30, math.random(4, 7) / 3), alpha = math.random(40, 70) / 100}
end
...
local function updateParticles()
    ...
    if tick() - lastParticleUpdate < 0.1 then return end
    ...
    for i, particle in ipairs(particles) do
        ...
        local newAlpha = particle.alpha + math.sin(i * 0.5 + tick()) * 0.08
        newAlpha = math.clamp(newAlpha, 0.3, 0.7)
        if newY > 1 or newX < -0.1 or newX > 1.1 then
            newX = math.random()
            newY = -0.1
            particle.velocity = Vector2.new(math.random(-2, 2) / 30, math.random(4, 7) / 3)
...
local function createToggle(parent, text, defaultValue, callback)
    ...
    label.Text = text
    label.TextColor3 = Color3.fromRGB(230, 220, 220)
    ...
    toggle.BackgroundColor3 = defaultValue and Color3.fromRGB(0, 220, 20) or Color3.fromRGB(220, 20, 0)
    ...
    toggle.MouseButton1Click:Connect(function()
        ...
        toggle.Text = defaultValue and "ON" or "OFF"
        local targetColor = defaultValue and Color3.fromRGB(50, 220, 50) or Color3.fromRGB(220, 50, 50)
    ...
    toggle.MouseLeave:Connect(function()
        ...
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(100, 150, 255)}):Play()
    end)
...
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    valueLabel.TextColor3 = Color3.fromRGB(230, 220, 220)
    ...
    prevButton.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    prevButton.Text = "<"
    prevButton.TextColor3 = Color3.fromRGB(230, 220, 220)
    ...
    nextButton.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    nextButton.Text = ">"
    nextButton.TextColor3 = Color3.fromRGB(230, 220, 220)
    ...
    local function updateValue()
        ...
        TweenService:Create(valueLabel, tweenInfo, {TextTransparency = 0.2}):Play()
    ...
    prevButton.MouseEnter:Connect(function()
        TweenService:Create(prevButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(40, 40, 50), Size = UDim2.new(0.11, 0, 0.85, 0)}):Play()
    end)
    
    prevButton.MouseLeave:Connect(function()
        TweenService:Create(prevButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(25, 25, 35), Size = UDim2.new(0.1, 0, 0.8, 0)}):Play()
    end)
    
    nextButton.MouseEnter:Connect(function()
        TweenService:Create(nextButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(40, 40, 50), Size = UDim2.new(0.11, 0, 0.85, 0)}):Play()
    end)
    
    nextButton.MouseLeave:Connect(function()
        TweenService:Create(nextButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(25, 25, 35), Size = UDim2.new(0.1, 0, 0.8, 0)}):Play()
    end)
...
local function createColorButton(parent, text, defaultColor, callback)
    ...
    colorButton.MouseEnter:Connect(function()
        ...
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(150, 200, 255), Thickness = 1.5}):Play()
    end)
    
    colorButton.MouseLeave:Connect(function()
        TweenService:Create(colorButton, tweenInfo, {Size = UDim2.new(0.3, 0, 0.8, 0)}):Play()
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(100, 150, 255), Thickness = 1}):Play()
    end)
...
local function isEnemy(player)
    ...
    local localRole = LocalPlayer:FindFirstChild("Data") and LocalPlayer.Data:FindFirstChild("Role") and LocalPlayer.Data.Role.Value
    local playerRole = player:FindFirstChild("Data") and player.Data:FindFirstChild("Role") and player.Data.Role.Value
    
    -- Для Murder vs. Sheriff
    if game.PlaceId == 4616656197 then -- PlaceId для Murder vs. Sheriff
        if player.Team and LocalPlayer.Team then
            return player.Team ~= LocalPlayer.Team
        end
        local hasWeapon = player.Character:FindFirstChild("Gun") or player.Character:FindFirstChild("Knife")
        return hasWeapon and player ~= LocalPlayer
    end
    
    -- Для Murder Mystery 2
    if not localRole or not playerRole then
        local hasWeapon = player.Character:FindFirstChild("Gun") or player.Character:FindFirstChild("Knife")
        return hasWeapon and player ~= LocalPlayer
    end
    
    if localRole == "Innocent" then
        return playerRole == "Murderer" or playerRole == "Sheriff"
    elseif localRole == "Sheriff" then
        return playerRole == "Murderer"
    elseif localRole == "Murderer" then
        return playerRole == "Innocent" or playerRole == "Sheriff"
    end
    return false
end
...
local function updateHitboxes()
    ...
    connections.Hitbox = RunService.Heartbeat:Connect(function()
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and (not settings.Combat.Hitbox.EnemiesOnly or isEnemy(player)) then
    ...
    connections.HitboxCheck = RunService.Heartbeat:Connect(function()
        ...
        if settings.Combat.Hitbox.Enabled then
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChildOfClass("Humanoid") and (not settings.Combat.Hitbox.EnemiesOnly or isEnemy(player)) and not hitboxHighlights[player] then
                    updateHitboxes()
                    break
                end
            end
...
local function updateESP()
    ...
    connections.ESP = RunService.Heartbeat:Connect(function()
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and (not settings.ESP.EnemiesOnly or isEnemy(player)) then
                local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
                local humanoidRootPart = player.Character:FindFirstChild("HumanoidRootPart")
                if humanoid and humanoidRootPart and humanoid.Health > 0 then
    ...
    connections.ESPCheck = RunService.Heartbeat:Connect(function()
        ...
        if settings.ESP.Enabled then
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChildOfClass("Humanoid") and (not settings.ESP.EnemiesOnly or isEnemy(player)) and settings.ESP.Boxes and not espBoxes[player] then
                    updateESP()
                    break
                end
            end
...
for i, tabName in ipairs(tabs) do
    ...
    tabButton.BackgroundColor3 = i == currentTab and Color3.fromRGB(50, 50, 80) or Color3.fromRGB(30, 30, 45)
    tabButton.BackgroundTransparency = 0.2
    tabButton.Text = tabName
    tabButton.TextColor3 = Color3.fromRGB(230, 220, 220)
    ...
    tabStroke.Color = i == currentTab and Color3.fromRGB(100, 150, 255) or Color3.fromRGB(50, 50, 80)
    ...
    if tabName == "Combat" then
        ...
        createToggle(scrollFrame, "Hitbox Enemies Only", settings.Combat.Hitbox.EnemiesOnly, function(value)
            settings.Combat.Hitbox.EnemiesOnly = value
            updateHitboxes()
        end)
    ...
    elseif tabName == "ESP" then
        ...
        createToggle(scrollFrame, "ESP Enemies Only", settings.ESP.EnemiesOnly, function(value)
            settings.ESP.EnemiesOnly = value
            updateESP()
        end)
    ...
    tabButton.MouseButton1Click:Connect(function()
        ...
        for j, btn in ipairs(tabButtons) do
            ...
            btn.BackgroundColor3 = isActive and Color3.fromRGB(50, 50, 80) or Color3.fromRGB(30, 30, 45)
            local stroke = btn:FindFirstChildOfClass("UIStroke")
            if stroke then
                stroke.Color = isActive and Color3.fromRGB(100, 150, 255) or Color3.fromRGB(50, 50, 80)
    ...
    tabButton.MouseLeave:Connect(function()
        if i ~= currentTab then
            TweenService:Create(tabButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(30, 30, 45)}):Play()
        end
    end)
end
...
dragArea.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(dragArea, tweenInfo, {TextTransparency = isMinimized and 0.3 or 0.1, TextSize = isMinimized and 13 or 16}):Play()
end)
...
RunService:BindToRenderStep("MenuUpdate", Enum.RenderPriority.Input.Value + 1, function()
    ...
    for i, btn in ipairs(tabButtons) do
        local targetColor = i == currentTab and Color3.fromRGB(50, 50, 80) or Color3.fromRGB(30, 30, 45)
