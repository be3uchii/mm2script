...
for i = 1, maxParticles do
    ...
    particle.BackgroundColor3 = Color3.fromRGB(220, 220, 220)
    ...
    particles[i] = {frame = particle, velocity = Vector2.new(math.random(-4, 4) / 15, math.random(6, 10) / 10), alpha = math.random(30, 60) / 100}
end
...
local function updateParticles()
    ...
    for _, particle in ipairs(particles) do
        ...
        local newAlpha = particle.alpha + math.sin(tick() * 2.5 + newX * 12) * 0.1
        newAlpha = math.clamp(newAlpha, 0.3, 0.6)
        if newY > 1.5 or newX < -0.1 or newX > 1.1 then
            newX = math.random()
            newY = -0.1
            particle.velocity = Vector2.new(math.random(-4, 4) / 15, math.random(6, 10) / 10)
...
local function createToggle(parent, text, defaultValue, callback)
    ...
    toggle.MouseButton1Click:Connect(function()
        ...
        local tween = TweenService:Create(toggle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
            BackgroundColor3 = newValue and Color3.fromRGB(0, 170, 0) or Color3.fromRGB(170, 0, 0),
            Text = newValue and "ON" or "OFF"
        })
        tween:Play()
        callback(newValue)
    end)
...
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    local function updateValue()
        ...
        local tween = TweenService:Create(valueLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {TextTransparency = 0.2})
        tween:Play()
        tween.Completed:Connect(function()
            local fadeOut = TweenService:Create(valueLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {TextTransparency = 0.4})
            fadeOut:Play()
        end)
    ...
    prevButton.MouseButton1Click:Connect(function()
        ...
        local tween = TweenService:Create(prevButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = Color3.fromRGB(80, 80, 80)})
        tween:Play()
        tween.Completed:Connect(function()
            local reset = TweenService:Create(prevButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = Color3.fromRGB(50, 50, 50)})
            reset:Play()
        end)
        updateValue()
    end)
    ...
    nextButton.MouseButton1Click:Connect(function()
        ...
        local tween = TweenService:Create(nextButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = Color3.fromRGB(80, 80, 80)})
        tween:Play()
        tween.Completed:Connect(function()
            local reset = TweenService:Create(nextButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = Color3.fromRGB(50, 50, 50)})
            reset:Play()
        end)
        updateValue()
    end)
...
local function createColorButton(parent, text, defaultColor, callback)
    ...
    colorButton.MouseButton1Click:Connect(function()
        ...
        local tween = TweenService:Create(colorButton, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = colors[currentColorIndex]})
        tween:Play()
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if humanoid and humanoidRootPart then
                    ...
                    humanoidRootPart.CanCollide = not settings.Misc.NoClip
