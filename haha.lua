    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if humanoidRootPart then
                ...
                selection.Color3 = settings.Combat.Hitbox.Color
                selection.Transparency = settings.Combat.Hitbox.Transparency
                selection.LineThickness = 0.05
                selection.Parent = hitbox
                
                if settings.ESP.ThroughWalls then
...
local function autoStab()
    if not settings.Combat.AutoStab then return end
...
local function autoArrest()
    if not settings.Combat.AutoArrest then return end
        ...
        if player ~= LocalPlayer and player.Character then
            ...
            if humanoidRootPart and humanoid and humanoid.Health > 0 then
                ...
                if distance < 10 then
                    ...
                    game:GetService("ReplicatedStorage"):FindFirstChild("Arrest"):FireServer(unpack(args))
                    task.wait(1)
                end
            end
        end
...
local function murdererAura()
    if not settings.Combat.MurdererAura then return end
...
local function sheriffAura()
    if not settings.Combat.SheriffAura then return end
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        createToggle(scrollFrame, "Murderer Aura (MM2)", settings.Combat.MurdererAura, function(value)
            settings.Combat.MurdererAura = value
            clearConnections()
            if value then
                table.insert(connections, RunService.Heartbeat:Connect(murdererAura))
            end
        end)
        
        createToggle(scrollFrame, "Sheriff Aura (MM2)", settings.Combat.SheriffAura, function(value)
            settings.Combat.SheriffAura = value
            clearConnections()
            if value then
                table.insert(connections, RunService.Heartbeat:Connect(sheriffAura))
            end
        end)
        
        createToggle(scrollFrame, "Auto Stab (MvS)", settings.Combat.AutoStab, function(value)
            settings.Combat.AutoStab = value
            clearConnections()
            if value then
                table.insert(connections, RunService.Heartbeat:Connect(autoStab))
            end
        end)
        
        createToggle(scrollFrame, "Auto Arrest (MvS)", settings.Combat.AutoArrest, function(value)
            settings.Combat.AutoArrest = value
            clearConnections()
            if value then
                table.insert(connections, RunService.Heartbeat:Connect(autoArrest))
            end
        end)
    elseif tabName == "ESP" then
        ...
        createToggle(scrollFrame, "Through Walls", settings.ESP.ThroughWalls, function(value)
            settings.ESP.ThroughWalls = value
            updateHitboxes()
            updateESP()
        end)
