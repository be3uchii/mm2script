...
local VisualEffects = {
    BlackSky = {Active = false, Apply = function(s) Lighting.Sky.SkyboxBk = s.Active and "rbxassetid://7018684000" or "" end},
    NoFog = {Active = false, Apply = function(s) Lighting.FogEnd = s.Active and 100000 or 1000 end},
    BrightNight = {Active = false, Apply = function(s) Lighting.Brightness = s.Active and 2 or 1; Lighting.ClockTime = s.Active and 0 or 14 end},
    NoShadows = {Active = false, Apply = function(s) Lighting.GlobalShadows = not s.Active end},
    PlayerGlow = {Active = false, Apply = function(s) 
        local char = LocalPlayer.Character
        if char then
            for _,v in pairs(char:GetDescendants()) do
                if v:IsA("BasePart") then
                    local light = v:FindFirstChild("PlayerGlow")
                    if s.Active and not light then
                        ...
                        light.Parent = v
                    elseif not s.Active and light then
                        light:Destroy()
                    end
                end
            end
    ...
    RainbowWorld = {Active = false, Apply = function(s)
        if s.Active then
    ...
    NoClouds = {Active = false, Apply = function(s)
        for _,v in pairs(Lighting:GetChildren()) do
            if v:IsA("Clouds") then
                v.Enabled = not s.Active
            end
        end
    end},
    DarkWater = {Active = false, Apply = function(s) Lighting.WaterColor = s.Active and Color3.fromRGB(10,20,30) or Color3.fromRGB(14,22,39) end},
    Fullbright = {Active = false, Apply = function(s) Lighting.Ambient = s.Active and Color3.new(1,1,1) or Color3.new(0.5,0.5,0.5) end},
    NightVision = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("ColorCorrectionEffect")
                s.Effect.Parent = Lighting
            end
            s.Effect.TintColor = Color3.fromRGB(0,255,0)
            s.Effect.Contrast = 0.5
            s.Effect.Brightness = 0.3
        elseif s.Effect then
            s.Effect.TintColor = Color3.new(1,1,1)
            s.Effect.Contrast = 0
            s.Effect.Brightness = 0
        end
    end},
    ThermalVision = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("ColorCorrectionEffect")
                s.Effect.Parent = Lighting
            end
            s.Effect.TintColor = Color3.fromRGB(255,0,0)
            s.Effect.Contrast = 1
            s.Effect.Brightness = 0.2
        elseif s.Effect then
            s.Effect.TintColor = Color3.new(1,1,1)
            s.Effect.Contrast = 0
            s.Effect.Brightness = 0
    ...
    Vignette = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("ColorCorrectionEffect")
                s.Effect.Name = "VignetteEffect"
                s.Effect.Parent = Lighting
            end
            s.Effect.Enabled = true
        elseif s.Effect then
            s.Effect.Enabled = false
        end
    end},
    Bloom = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("BloomEffect")
                s.Effect.Parent = Lighting
                s.Effect.Intensity = 0.5
                s.Effect.Size = 24
            end
            s.Effect.Enabled = true
        elseif s.Effect then
    ...
    PlayerOutline = {Active = false, Outlines = {}, Apply = function(s)
        local char = LocalPlayer.Character
        if char then
            if s.Active then
                for _,v in pairs(char:GetDescendants()) do
                    if v:IsA("BasePart") then
                        ...
                        outline.Parent = v
                        table.insert(s.Outlines, outline)
                    end
                end
            else
                for _,v in pairs(s.Outlines) do v:Destroy() end
                s.Outlines = {}
            end
        end
    end},
    TimeStop = {Active = false, Apply = function(s) Lighting.ClockTime = s.Active and 12 or 14 end},
    Underwater = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("PostEffect")
                s.Effect.Name = "UnderwaterEffect"
                s.Effect.Parent = Lighting
            end
            s.Effect.Enabled = true
        elseif s.Effect then
    ...
    Grayscale = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("ColorCorrectionEffect")
                s.Effect.Parent = Lighting
            end
            s.Effect.Saturation = -1
        elseif s.Effect then
            s.Effect.Saturation = 0
        end
    end},
    Pixelation = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("PixelateEffect")
                s.Effect.Parent = Lighting
            end
            s.Effect.Enabled = true
        elseif s.Effect then
    ...
    SunRays = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("SunRaysEffect")
                s.Effect.Parent = Lighting
                s.Effect.Intensity = 0.2
            end
            s.Effect.Enabled = true
        elseif s.Effect then
            s.Effect.Enabled = false
    ...
    Chromatic = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("ChromaticAberrationEffect")
                s.Effect.Parent = Lighting
            end
            s.Effect.Enabled = true
        elseif s.Effect then
    ...
    DepthOfField = {Active = false, Effect = nil, Apply = function(s)
        if s.Active then
            if not s.Effect then
                s.Effect = Instance.new("DepthOfFieldEffect")
                s.Effect.Parent = Lighting
            end
            s.Effect.Enabled = true
        elseif s.Effect then
...
for name, effect in pairs(VisualEffects) do
    ...
    button.MouseButton1Click:Connect(function()
        ...
        button.BackgroundColor3 = effect.Active and Color3.fromRGB(0,150,200) or Color3.fromRGB(40,40,45)
        button.TextColor3 = effect.Active and Color3.fromRGB(255,255,255) or Color3.fromRGB(220,220,220)
    end)
end
...
local function SetAnimation(animName)
    ...
    if character then ApplyAnimation(character, animName) end
...
LocalPlayer.CharacterAdded:Connect(function(character)
    if currentAnimation then task.wait(1); ApplyAnimation(character, currentAnimation) end
    for _,effect in pairs(VisualEffects) do
        if effect.Active then effect:Apply(true) end
    end
end)
