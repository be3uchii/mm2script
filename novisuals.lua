...
local function SaveOriginalHipHeight(character)
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid then
        OriginalHipHeight[character] = humanoid.HipHeight
    end
end
...
local function Transform(transformationName)
    ...
    if not OriginalSizes[character] then
        ...
        SaveOriginalHipHeight(character)
    end
    
    if transformData.Speed then
        humanoid.WalkSpeed = transformData.Speed
        RemoteEvent:FireServer("Transform", transformationName, {Speed = transformData.Speed})
    end
    ...
    if transformData.Scale then
        ...
        if rootPart then
            ...
            for _, part in pairs(character:GetDescendants()) do
                if part:IsA("BasePart") then
                    part.Size = OriginalSizes[character][part] * transformData.Scale
                end
            end
            humanoid.HipHeight = OriginalHipHeight[character] * transformData.Scale
            rootPart.CFrame = currentCFrame
        end
        RemoteEvent:FireServer("Transform", transformationName, {Scale = transformData.Scale})
    end
end
...
Player.CharacterAdded:Connect(function(character)
    ...
    OriginalHipHeight[character] = nil
end)

CreateTabButtons()

RemoteEvent.OnClientEvent:Connect(function(action, data, senderId, transformData)
...
if game:GetService("RunService"):IsServer() then
    ...
    RemoteEvent.OnServerEvent:Connect(function(player, action, data, transformData)
        for _, otherPlayer in ipairs(Players:GetPlayers()) do
            if otherPlayer ~= player then
                RemoteEvent:FireClient(otherPlayer, action, data, player.UserId, transformData)
            end
        end
    end)
end