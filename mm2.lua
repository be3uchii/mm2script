...
local function getTargetInFOV()
    if not settings.FOV.Enabled then return nil end
...
createCheckbox("FOVEnabled", "Включить FOV", UDim2.new(0, 0, 0, 0), fovTab, settings.FOV, "Enabled")
createSlider("FOVSize", "Размер FOV", UDim2.new(0, 0, 0, 40), fovTab, settings.FOV, "Size", 50, 300)
createSlider("FOVThickness", "Толщина FOV", UDim2.new(0, 0, 0, 110), fovTab, settings.FOV, "Thickness", 1, 10)
createCheckbox("FOVFilled", "Заполненный FOV", UDim2.new(0, 0, 0, 180), fovTab, settings.FOV, "Filled")

createCheckbox("AimEnabled", "Включить АИМ", UDim2.new(0, 0, 0, 0), aimTab, settings.AIM, "Enabled")
createDropdown("AimTarget", "Цель АИМ", UDim2.new(0, 0, 0, 50), aimTab, settings.AIM, "Target", {"Murderer", "Sheriff"})
createSlider("AimSmoothness", "Плавность АИМ", UDim2.new(0, 0, 0, 130), aimTab, settings.AIM, "Smoothness", 0.1, 1)

createCheckbox("ESPEnabled", "Включить ESP", UDim2.new(0, 0, 0, 0), espTab, settings.ESP, "Enabled")
createSlider("ESPThickness", "Толщина ESP", UDim2.new(0, 0, 0, 50), espTab, settings.ESP, "Thickness", 1, 5)
    ...
    if method == "FireServer" and (tostring(self) == "HitPart" or tostring(self) == "KnifeHit") then
        ...
        if target and target:IsA("BasePart") and target.Parent then
            ...
            if character:FindFirstChild("Humanoid") then
                if settings.FOV.Enabled and hasWeapon() then
