...
local TouchInputService = game:GetService("UserInputService") -- Для сенсорного ввода
...
-- Заголовок (также используется как кнопка сворачивания)
local Title = Instance.new("TextButton") -- Изменено на TextButton для сенсорного управления
...
local function CreateOption(parent, text, settingKey, isSlider, minValue, maxValue, defaultValue)
    ...
    Toggle.TouchTap:Connect(toggleAction) -- Поддержка сенсорных касаний
...
for tabName, button in pairs(tabButtons) do
    ...
    button.TouchTap:Connect(function() -- Поддержка сенсорных касаний
        currentTab = tabName
        for name, scroll in pairs(tabScrolls) do
            scroll.Visible = name == tabName
        end
...
Title.TouchTap:Connect(function() -- Поддержка сенсорных касаний
    Minimized = not Minimized
    local newSize = Minimized and UDim2.new(0, 400, 0, 40) or UDim2.new(0, 400, 0, 300)
    TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = newSize}):Play()
    TabButtons.Visible = not Minimized
...
-- Горячая клавиша для открытия/закрытия меню (для ПК, на случай если используется эмулятор)
...
-- Поддержка сенсорного управления для сворачивания
