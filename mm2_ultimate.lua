    ...
    for player, data in pairs(espData) do
        if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if (role == "Murderer" and not espMurderer) or (role == "Sheriff" and not espSheriff) or (role == "Innocent" and not espInnocent) then
                ...
                data.distanceLabel.Text = espDistance and tostring(math.floor((LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart and (LocalPlayer.Character.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position).Magnitude or 0)) .. "m" or ""
            end
        end
    end
end
...
local function killAura()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
            local distance = (LocalPlayer.Character.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position).Magnitude
            if distance <= killAuraRange then
                local knifeHit = game:GetService("ReplicatedStorage"):FindFirstChild("GameEvents") and game:GetService("ReplicatedStorage").GameEvents:FindFirstChild("KnifeHit")
                if knifeHit then
                    knifeHit:FireServer(player)
                end
            end
        end
    end
            ...
            if role == "Murderer" then
                ...
                if onScreen then
                    ...
                    if distance <= fovSize / 2 then
                        local gunShot = game:GetService("ReplicatedStorage"):FindFirstChild("GameEvents") and game:GetService("ReplicatedStorage").GameEvents:FindFirstChild("GunShot")
                        if gunShot then
                            gunShot:FireServer(player.Character.HumanoidRootPart.Position)
                        end
                    end
                end
            end
...
UserInputService.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch and autoShootEnabled then
        autoShoot()
    end
end)
...
playerTabButton.Activated:Connect(function()
    playerTab.Visible = true
    murdererTab.Visible = false
    sheriffTab.Visible = false
    generalTab.Visible = false
...
murdererTabButton.Activated:Connect(function()
...
sheriffTabButton.Activated:Connect(function()
    playerTab.Visible = false
    murdererTab.Visible = false
...
generalTabButton.Activated:Connect(function()
    playerTab.Visible = false
    murdererTab.Visible = false
    sheriffTab.Visible = false
