...
-- Заголовок (для сворачивания/разворачивания)
...
for i, tabName in ipairs(tabs) do
    ...
    ScrollFrame.ScrollBarThickness = 5 -- Увеличена толщина полосы прокрутки
    ScrollFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
    ScrollFrame.Visible = tabName == currentTab
    ScrollFrame.Parent = MainFrame
    ScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 0) -- Будет обновляться динамически
...
local function CreateOption(parent, text, settingKey, isSlider, minValue, maxValue, defaultValue)
    ...
    -- Обновление размера Canvas для прокрутки
...
setupTouchDrag(Title) -- Добавлено перетаскивание через заголовок
