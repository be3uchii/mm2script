...
ProgressBarFrame.Size = UDim2.new(0.4, 0, 0, 20)
ProgressBarFrame.Position = UDim2.new(0.3, 0, 0.5, 0)
...
local function animateLoading()
    ...
    while tick() - startTime < duration do
        ...
        TweenService:Create(ProgressBar, TweenInfo.new(0.1, Enum.EasingStyle.Sine), {Size = UDim2.new(progress, 0, 1, 0)}):Play()
        -- Пульсация текста
        TweenService:Create(LoadingText, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {TextTransparency = 0.2 + 0.6 * math.sin(math.pi * 2 * progress)}):Play()
        -- Градиент цвета прогресс-бара
        TweenService:Create(ProgressBar, TweenInfo.new(0.5, Enum.EasingStyle.Linear), {BackgroundColor3 = Color3.fromRGB(0, 170 + 85 * math.sin(math.pi * 2 * progress), 255)}):Play()
        wait(0.1)
    end
    -- Завершение анимации
    TweenService:Create(ProgressBar, TweenInfo.new(0.3, Enum.EasingStyle.Sine), {Size = UDim2.new(1, 0, 1, 0)}):Play()
    TweenService:Create(LoadingText, TweenInfo.new(0.5, Enum.EasingStyle.Sine), {TextTransparency = 1}):Play()
    wait(0.5)
    LoadingScreen:Destroy()
    -- Плавное появление меню
    MainFrame.BackgroundTransparency = 1
    MainFrame.Visible = true
    TweenService:Create(MainFrame, TweenInfo.new(0.5, Enum.EasingStyle.Sine), {BackgroundTransparency = 0.1}):Play()
end
...
MainFrame.BackgroundTransparency = 1 -- Скрыт до появления
...
local function CreateSliderOption(parent, text, settingKey, minValue, maxValue, defaultValue)
    ...
    SliderBar.Size = UDim2.new((Settings[settingKey] - minValue) / (maxValue - minValue), 0, 1, 0)
...
local function KnifeThrowSpeed()
    if Settings.KnifeThrowSpeed and Player.Character then
        local knife = Player.Character:FindFirstChild("Knife")
        if knife then
            knife.Velocity = knife.Velocity * 1.5
        end
    end
end
