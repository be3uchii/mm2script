...
uiGradient.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(50, 50, 80)),
...
dragArea.TextColor3 = Color3.fromRGB(200, 200, 255)
...
for i = 1, maxParticles do
    ...
    particles[i] = {frame = particle, velocity = Vector2.new(math.random(-3, 3) / 40, math.random(3, 7) / 30), alpha = math.random(30, 60) / 100}
end
...
local function updateParticles()
    ...
    for _, particle in ipairs(particles) do
        ...
        local newX = pos.X.Scale + particle.velocity.X + math.sin(tick() * 2) * 0.01
        local newY = pos.Y.Scale + particle.velocity.Y + math.cos(tick() * 1.5) * 0.01
        local newAlpha = particle.alpha + math.sin(tick() * 1.5 + newX * 10) * 0.05
        newAlpha = math.clamp(newAlpha, 0.3, 0.5)
        if newY > 1.5 or newX < -0.1 or newX > 1.1 then
            newX = math.random()
            newY = -0.2
            particle.velocity = Vector2.new(math.random(-3, 3) / 40, math.random(3, 7) / 30)
            particle.alpha = math.random(30, 60) / 100
        end
        particle.frame.Position = UDim2.new(newX, 0, newY, 0)
        particle.frame.BackgroundTransparency = newAlpha
        TweenService:Create(particle.frame, tweenInfo, {Rotation = math.sin(tick() * 2) * 15, Size = UDim2.new(0, size * (1 + math.sin(tick() * 2) * 0.1), 0, size * (1 + math.sin(tick() * 2) * 0.1))}):Play()
    end
...
local function createToggle(parent, text, defaultValue, callback)
    ...
    toggle.MouseLeave:Connect(function()
        ...
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(100, 100, 255)}):Play()
    end)
    return toggleFrame
end
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    prevButton.Text = "<"
    prevButton.TextColor3 = Color3.fromRGB(200, 200, 255)
    ...
    nextButton.Text = ">"
    nextButton.TextColor3 = Color3.fromRGB(200, 200, 255)
    ...
    prevButton.MouseEnter:Connect(function()
        TweenService:Create(prevButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(50, 50, 80), Size = UDim2.new(0.105, 0, 0.82, 0), Position = UDim2.new(0.7, 0, 0.08, 0)}):Play()
    end)
    ...
    nextButton.MouseEnter:Connect(function()
        TweenService:Create(nextButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(50, 50, 80), Size = UDim2.new(0.105, 0, 0.82, 0), Position = UDim2.new(0.85, 0, 0.08, 0)}):Play()
    end)
...
local function createColorButton(parent, text, defaultColor, callback)
    ...
    colorButton.MouseEnter:Connect(function()
        ...
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(150, 150, 255), Thickness = 1.7}):Play()
    end)
    colorButton.MouseLeave:Connect(function()
        TweenService:Create(colorButton, tweenInfo, {Size = UDim2.new(0.3, 0, 0.8, 0), Position = UDim2.new(0.7, 0, 0.1, 0)}):Play()
        TweenService:Create(stroke, tweenInfo, {Color = Color3.fromRGB(100, 100, 255), Thickness = 1.5}):Play()
    end)
    return colorFrame
end
local function createTransparencySlider(parent, text, defaultValue, callback)
    ...
    prevButton.MouseEnter:Connect(function()
        TweenService:Create(prevButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(50, 50, 80), Size = UDim2.new(0.105, 0, 0.82, 0), Position = UDim2.new(0.65, 0, 0.08, 0)}):Play()
    end)
    prevButton.MouseLeave:Connect(function()
...
for i, tabName in ipairs(tabs) do
    ...
    tabStroke.Color = i == currentTab and Color3.fromRGB(100, 100, 255) or Color3.fromRGB(40, 40, 60)
    ...
    tabShadow.Offset = Vector2.new(1, 1)
    tabShadow.Transparency = 0.3
    ...
    contentStroke.Color = Color3.fromRGB(100, 100, 255)
    ...
    contentShadow.Offset = Vector2.new(1, 1)
    contentShadow.Transparency = 0.3
    ...
    tabButton.MouseButton1Click:Connect(function()
        ...
        for j, frame in ipairs(contentFrames) do
            frame.Visible = j == i
            TweenService:Create(frame, tweenInfo, {BackgroundTransparency = j == i and 0.2 or 0.5, Position = j == i and UDim2.new(0, 4, 0, 55) or UDim2.new(0, 4, 0, 60)}):Play()
        end
        for j, btn in ipairs(tabButtons) do
            ...
            if stroke then
                stroke.Color = isActive and Color3.fromRGB(100, 100, 255) or Color3.fromRGB(40, 40, 60)
    ...
    tabButton.MouseEnter:Connect(function()
        if i ~= currentTab then
            TweenService:Create(tabButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(50, 50, 80), Position = UDim2.new((i - 1) / #tabs, 1.5, 0, 1)}):Play()
        end
    end)
...
TweenService:Create(mainFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.2}):Play()
...
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.Insert then
        ...
        screenGui.Enabled = isVisible
        TweenService:Create(mainFrame, tweenInfo, {BackgroundTransparency = isVisible and 0.2 or 1}):Play()
        if isVisible then
            updateHitboxes()
            updateESP()
        end
...
RunService:BindToRenderStep("MenuUpdate", Enum.RenderPriority.Input.Value + 1, function()
    ...
    for i, btn in ipairs(tabButtons) do
        ...
        if scrollFrame then
            TweenService:Create(scrollFrame, tweenInfo, {ScrollBarImageColor3 = Color3.fromRGB(100 + math.sin(tick() * 2) * 20, 100 + math.sin(tick() * 2) * 20, 255)}):Play()
        end
    end
end)
