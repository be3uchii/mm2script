...
local function loadConfig(configName)
    ...
    local success, result = pcall(function()
        return HttpService:JSONEncode(readfile(configPath))
    end)
...
local function createModernToggle(parent, text, value, callback, id)
    ...
    gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, value and Color3.fromRGB(86, 227, 110) or Color3.fromRGB(255, 78, 78)),
        ColorSequenceKeypoint.new(1, value and Color3.fromRGB(66, 207, 90) or Color3.fromRGB(235, 38, 38))
    })
    gradient.Rotation = 90
    gradient.Parent = SwitchButton
    ...
    stroke.Parent = SwitchButton
    
    local hoverTween
    SwitchButton.MouseEnter:Connect(function()
        if hoverTween then hoverTween:Cancel() end
        hoverTween = TweenService:Create(SwitchButton, TweenInfo.new(0.1), {
...
local function createModernValueChanger(parent, text, values, value, callback, id)
    ...
    gradient.Rotation = 90
    gradient.Parent = valueFrame
    
    local stroke = Instance.new("UIStroke")
    stroke.Color = Color3.fromRGB(80, 80, 100)
    stroke.Thickness = 1
    stroke.Parent = valueFrame
    ...
    prevButton.MouseEnter:Connect(function()
        TweenService:Create(prevButton, TweenInfo.new(0.1), {
    ...
    nextButton.MouseEnter:Connect(function()
        TweenService:Create(nextButton, TweenInfo.new(0.1), {
            ImageTransparency = 0.1
        }):Play()
    end)
...
local function createModernColorButton(parent, text, value, callback, id)
    ...
    colorButton.MouseEnter:Connect(function()
        if hoverTween then hoverTween:Cancel() end
        hoverTween = TweenService:Create(colorButton, TweenInfo.new(0.1), {
    ...
    colorButton.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(colorButton, tweenInfo, {
            BackgroundColor3 = value,
            Size = UDim2.new(0.25, -2, 0.8, -2)
        }):Play()
        TweenService:Create(colorButton, tweenInfo, {
            Size = UDim2.new(0.25, 0, 0.8, 0)
        }):Play()
...
local function refreshCombatUI()
    ...
    createModernToggle(combatScrollFrame, "Hitbox Enabled", settings.Combat.Hitbox.Enabled, function(value)
    ...
    createModernValueChanger(combatScrollFrame, "Hitbox Size", sizeValues, settings.Combat.Hitbox.Size, function(value)
        settings.Combat.Hitbox.Size = value
        updateAllESP()
    end, "hitboxSize")
    
    createModernToggle(combatScrollFrame, "Sphere Hitbox", settings.Combat.Hitbox.Type == "Sphere", function(value)
        settings.Combat.Hitbox.Type = value and "Sphere" or "Box"
        clearHitboxes()
        updateAllESP()
    end, "hitboxType")
    
    createModernColorButton(combatScrollFrame, "Hitbox Color", settings.Combat.Hitbox.Color, function(value)
        settings.Combat.Hitbox.Color = value
        updateAllESP()
    end, "hitboxColor")
    
    createModernValueChanger(combatScrollFrame, "Hitbox Transparency", transparencies, settings.Combat.Hitbox.Transparency, function(value)
...
local function refreshESPUI()
    ...
    createModernToggle(espScrollFrame, "ESP Enabled", settings.ESP.Enabled, function(value)
        settings.ESP.Enabled = value
        updateAllESP()
    end, "espEnabled")
    
    createModernToggle(espScrollFrame, "Show Distance", settings.ESP.ShowDistance, function(value)
        settings.ESP.ShowDistance = value
        updateAllESP()
    end, "espShowDistance")
end

local function createModernConfigButton(parent, text, position, callback)
    ...
    button.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(button, TweenInfo.new(0.05, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
...
local function refreshConfigUI(scrollFrame)
    ...
    gradient.Rotation = 90
    gradient.Parent = createConfigFrame
    
    local stroke = Instance.new("UIStroke")
    stroke.Color = Color3.fromRGB(60, 60, 80)
    stroke.Thickness = 1
    stroke.Parent = createConfigFrame
    ...
    local createButton = createModernConfigButton(createConfigFrame, "Create", UDim2.new(0.75, 0, 0.2, 0), function()
    ...
    for _, configName in ipairs(scanConfigs()) do
        ...
        gradient:Clone().Parent = configFrame
        stroke:Clone().Parent = configFrame
        ...
        local loadButton = createModernConfigButton(configFrame, "Load", UDim2.new(0.65, 0, 0.15, 0), function()
        ...
        local deleteButton = createModernConfigButton(configFrame, "Delete", UDim2.new(0.85, 0, 0.15, 0), function()
...
local function createMainGUI()
    ...
    for i, tabName in ipairs(tabs) do
        ...
        local gradient = Instance.new("UIGradient")
        gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, i == currentTab and Color3.fromRGB(70, 70, 90) or Color3.fromRGB(50, 50, 70)),
            ColorSequenceKeypoint.new(1, i == currentTab and Color3.fromRGB(50, 50, 70) or Color3.fromRGB(30, 30, 50))
        })
        gradient.Rotation = 90
        ...
        tabButton.MouseButton1Click:Connect(function()
            ...
            for j, btn in ipairs(tabButtons) do
                ...
                gradient.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, j == currentTab and Color3.fromRGB(70, 70, 90) or Color3.fromRGB(50, 50, 70)),
                    ColorSequenceKeypoint.new(1, j == currentTab and Color3.fromRGB(50, 50, 70) or Color3.fromRGB(30, 30, 50))
                })
            end
    ...
    dragArea.MouseButton1Click:Connect(function()
        ...
        dragArea.TextSize = isMinimized and 12 or 14
        mainFrame.BackgroundTransparency = isMinimized and 0.5 or 0.3
        rightIcon.Visible = not isMinimized
    end)
