...
local function createToggle(parent, text, settingPath, callback)
    ...
    toggle.BackgroundColor3 = settings[settingPath[1]][settingPath[2]] and Color3.fromRGB(0, 230, 0) or Color3.fromRGB(230, 0, 0)
    toggle.Text = settings[settingPath[1]][settingPath[2]] and "ON" or "OFF"
    ...
    toggle.MouseButton1Click:Connect(function()
        settings[settingPath[1]][settingPath[2]] = not settings[settingPath[1]][settingPath[2]]
        toggle.Text = settings[settingPath[1]][settingPath[2]] and "ON" or "OFF"
        toggle.BackgroundColor3 = settings[settingPath[1]][settingPath[2]] and Color3.fromRGB(0, 230, 0) or Color3.fromRGB(230, 0, 0)
        if callback then callback(settings[settingPath[1]][settingPath[2]]) end
        saveSettings()
    end)
    
    uiElements[text] = toggle
    return toggleFrame
end

local function createValueChanger(parent, text, values, settingPath, callback)
    ...
    valueLabel.Text = tostring(settings[settingPath[1]][settingPath[2]])
    ...
    local currentIndex = table.find(values, settings[settingPath[1]][settingPath[2]]) or 1
    
    local function updateValue()
        settings[settingPath[1]][settingPath[2]] = values[currentIndex]
    ...
    uiElements[text] = {valueLabel = valueLabel, currentIndex = currentIndex, values = values}
    return changerFrame
end

local function createColorButton(parent, text, settingPath, callback)
    ...
    colorButton.BackgroundColor3 = settings[settingPath[1]][settingPath[2]]
    colorButton.Text = ""
    ...
    colorButton.MouseButton1Click:Connect(function()
        ...
        for i, color in ipairs(colors) do
            if color == settings[settingPath[1]][settingPath[2]] then
                currentIndex = i
                break
            end
        end
        ...
        loadButton.MouseButton1Click:Connect(function()
            if loadConfig(configName) then
                for name, element in pairs(uiElements) do
                    if string.find(name, "Hitbox Enabled") or string.find(name, "ESP Enabled") or string.find(name, "Show Distance") or string.find(name, "Through Walls") or string.find(name, "Sphere Hitbox") then
                        if element.Text then
                            element.Text = settings.Combat.Hitbox.Enabled and "ON" or "OFF"
                            element.BackgroundColor3 = settings.Combat.Hitbox.Enabled and Color3.fromRGB(0, 230, 0) or Color3.fromRGB(230, 0, 0)
                        end
                    elseif string.find(name, "Hitbox Size") then
                        element.valueLabel.Text = tostring(settings.Combat.Hitbox.Size)
                        element.currentIndex = table.find(element.values, settings.Combat.Hitbox.Size) or 1
                    elseif string.find(name, "Hitbox Transparency") then
                        element.valueLabel.Text = tostring(settings.Combat.Hitbox.Transparency)
                        element.currentIndex = table.find(element.values, settings.Combat.Hitbox.Transparency) or 1
                    elseif string.find(name, "Hitbox Color") then
                        element.BackgroundColor3 = settings.Combat.Hitbox.Color
                    end
                end
                updateAllESP()
            end
...
local function createMainGUI()
    ...
    for i, tabName in ipairs(tabs) do
        ...
        if tabName == "Combat" then
            createToggle(scrollFrame, "Hitbox Enabled", {"Combat", "Hitbox", "Enabled"}, function(value)
            ...
            createValueChanger(scrollFrame, "Hitbox Size", sizeValues, {"Combat", "Hitbox", "Size"}, function(value)
                settings.Combat.Hitbox.Size = value
                updateAllESP()
            end)
            
            createToggle(scrollFrame, "Sphere Hitbox", {"Combat", "Hitbox", "Type"}, function(value)
                settings.Combat.Hitbox.Type = value and "Sphere" or "Box"
                clearHitboxes()
                updateAllESP()
            end)
            
            createToggle(scrollFrame, "Through Walls", {"Combat", "Hitbox", "ThroughWalls"}, function(value)
                settings.Combat.Hitbox.ThroughWalls = value
                updateAllESP()
            end)
            
            createColorButton(scrollFrame, "Hitbox Color", {"Combat", "Hitbox", "Color"}, function(value)
                settings.Combat.Hitbox.Color = value
                updateAllESP()
            end)
            
            createValueChanger(scrollFrame, "Hitbox Transparency", transparencies, {"Combat", "Hitbox", "Transparency"}, function(value)
                settings.Combat.Hitbox.Transparency = value
                updateAllESP()
            end)
        elseif tabName == "ESP" then
            createToggle(scrollFrame, "ESP Enabled", {"ESP", "Enabled"}, function(value)
                settings.ESP.Enabled = value
                updateAllESP()
            end)
            
            createToggle(scrollFrame, "Show Distance", {"ESP", "ShowDistance"}, function(value)
