...
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    uiStroke.Thickness = 1
    uiStroke.Transparency = 0.2
    uiStroke.Parent = prevButton
    uiStroke:Clone().Parent = nextButton
...
local function createESP(player)
    ...
    local function onCharacterAdded(character)
        ...
        playerConnections[player].died = humanoid.Died:Connect(function()
            ...
            if frozenPlayers[character] then
...
local function updateHighlight(character)
    ...
    if highlightCache[character] then
        highlightCache[character].Enabled = settings.ESP.Chams
        highlightCache[character].FillColor = settings.ESP.ChamsColor
        highlightCache[character].OutlineColor = settings.ESP.ChamsColor
    end
end
...
local function damagePlayer(targetChar)
...
for i, tabName in ipairs(tabs) do
    ...
    contentFrame.Position = UDim2.new(0, 4, 0, 46)
    ...
    tabButton.MouseButton1Click:Connect(function()
        ...
        for j, btn in ipairs(tabButtons) do
            btn.BackgroundColor3 = j == currentTab and Color3.fromRGB(70, 70, 90) or Color3.fromRGB(35, 35, 45)
        end
    end)
end
