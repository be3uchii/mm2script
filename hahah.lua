...
local function updateSkeletons()
    ...
    for player, skeleton in pairs(skeletons) do
        if player and player ~= LocalPlayer and player.Character then
            ...
            local distance = (player.Character:FindFirstChild("HumanoidRootPart").Position - camera.CFrame.Position).Magnitude
            if distance > settings.ESP.MaxDistance then
                for _, line in pairs(skeleton) do
                    line.Visible = false
                end
...
local function refreshESPUI()
    ...
    createCleanToggle(espScrollFrame, "Show Skeleton", settings.Skeleton.Enabled, function(value)
