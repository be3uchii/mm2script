...
local function CreateESP(player)
    if ESP.Tracers[player] or player == LocalPlayer then return end
    
    ESP.Tracers[player] = Drawing.new("Line")
    ESP.Tracers[player].Visible = false
    ESP.Tracers[player].Color = Color3.fromRGB(255, 255, 255)
    ESP.Tracers[player].Thickness = 1
    ESP.Tracers[player].ZIndex = 1

    local function onCharacterAdded(character)
        local humanoid = character:WaitForChild("Humanoid")
        
        ESP.Connections[player] = humanoid.Died:Connect(function()
            if ESP.Tracers[player] then
                ESP.Tracers[player].Visible = false
            end
        end)
        
        ESP.Connections[player.."Health"] = humanoid:GetPropertyChangedSignal("Health"):Connect(function()
            if ESP.Tracers[player] then
                ESP.Tracers[player].Visible = humanoid.Health > 0 and settings.ESP.Enabled and settings.ESP.ShowTracers
            end
        end)
    end
...
local function RemoveESP(player)
    ...
    if ESP.Connections[player.."Health"] then
        ESP.Connections[player.."Health"]:Disconnect()
        ESP.Connections[player.."Health"] = nil
    end
end

local function UpdateESP()
    ESP.Cache.ViewportSize = Camera.ViewportSize
    ESP.Cache.CameraCFrame = Camera.CFrame
    local cameraPos = ESP.Cache.CameraCFrame.Position
    local viewportCenter = Vector2.new(ESP.Cache.ViewportSize.X/2, ESP.Cache.ViewportSize.Y)
