...
local menuWidth, menuHeight = 350, 200
...
local function updateESP(player)
    ...
    local boxPosition = Vector2.new(rootPos.X - width/2, feetPos.Y)
    local boxSize = Vector2.new(width, height)
    
    if not espData.smoothPosition then
        espData.smoothPosition = boxPosition
    else
        espData.smoothPosition = lerpVector2(espData.smoothPosition, boxPosition, 0.3)
    end
    
    box.Size = boxSize
    box.Position = espData.smoothPosition
...
local function refreshESPUI()
    ...
    createValueChanger(espScrollFrame, "ESP Transparency", transparencies, settings.ESP.Transparency, function(value)
        ...
        loadButton.MouseButton1Click:Connect(function()
            ...
            if loadConfig(configName) then
                for id, element in pairs(uiElements) do
                    ...
                    elseif id == "espTransparency" then
                        element.update(settings.ESP.Transparency)
...
local function refreshChatUI(scrollFrame)
    ...
    for _, message in ipairs(settings.Chat.Messages) do
        local messageFrame = Instance.new("Frame")
        messageFrame.Size = UDim2.new(1, -8, 0, 30)
        messageFrame.Position = UDim2.new(0, 4, 0, yOffset)
...
local function addChatMessage(message)
    table.insert(settings.Chat.Messages, message)
    if #settings.Chat.Messages > 50 then
        table.remove(settings.Chat.Messages, 1)
    end
    settings.Chat.LastMessageTime = os.time()
    saveSettings()
    
    if scrollFrames[4] then
        refreshChatUI(scrollFrames[4])
    end
end

local function setupChat()
    local chatEvent = Instance.new("RemoteEvent")
    chatEvent.Name = "SilenceChatEvent"
    chatEvent.Parent = ReplicatedStorage
    
    chatEvent.OnClientEvent:Connect(function(message, sender)
...
local function createMainGUI()
    ...
    for i, tabName in ipairs(tabs) do
        ...
        elseif tabName == "Chat" then
            ...
            chatInputFrame.Size = UDim2.new(1, -8, 0, 30)
            chatInputFrame.Position = UDim2.new(0, 4, 1, -35)
            chatInputFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
            chatInputFrame.BackgroundTransparency = 0.1
            chatInputFrame.Parent = contentFrame

            local corner = Instance.new("UICorner")
            corner.CornerRadius = UDim.new(0, 6)
            corner.Parent = chatInputFrame

            local glow = Instance.new("UIStroke")
            glow.Color = Color3.fromRGB(60, 60, 255)
            glow.Thickness = 1
            glow.Transparency = 0.7
            glow.Parent = chatInputFrame

            chatInput = Instance.new("TextBox")
            chatInput.Size = UDim2.new(0.7, 0, 0.8, 0)
            chatInput.Position = UDim2.new(0.05, 0, 0.1, 0)
            chatInput.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
            chatInput.TextColor3 = Color3.fromRGB(230, 230, 230)
