...
local menuWidth, menuHeight = 250, 160
...
local minimizedSize = UDim2.new(0, 70, 0, 25)
local minimizedPosition = UDim2.new(0.5, -35, 0, 8)
...
local aimAssistCircle = nil
local targetPlayer = nil

local function createAimAssistCircle()
    if aimAssistCircle then return end
    
    aimAssistCircle = Drawing.new("Circle")
    aimAssistCircle.Visible = false
    aimAssistCircle.Color = settings.Combat.AimAssist.Color
    aimAssistCircle.Transparency = settings.Combat.AimAssist.Transparency
    aimAssistCircle.Thickness = 1
    aimAssistCircle.Filled = false
    aimAssistCircle.Position = Vector2.new(viewportSize.X/2, viewportSize.Y/2)
    aimAssistCircle.Radius = settings.Combat.AimAssist.Radius
end

local function updateAimAssist()
    if not aimAssistCircle or not settings.Combat.AimAssist.Enabled then return end
    
    aimAssistCircle.Visible = settings.Combat.AimAssist.Enabled
    aimAssistCircle.Color = settings.Combat.AimAssist.Color
    aimAssistCircle.Transparency = settings.Combat.AimAssist.Transparency
    aimAssistCircle.Radius = settings.Combat.AimAssist.Radius
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if humanoidRootPart then
                    ...
                    if onScreen then
                        local distance = (Vector2.new(screenPoint.X, screenPoint.Y) - Vector2.new(viewportSize.X/2, viewportSize.Y/2)).Magnitude
                        if distance <= settings.Combat.AimAssist.Radius then
                            targetPlayer = player
                            break
                        end
                    end
...
dragArea.Size = UDim2.new(1, 0, 0, 22)
dragArea.BackgroundTransparency = 1
dragArea.Text = "Silence 3.5"
dragArea.TextColor3 = Color3.fromRGB(200, 200, 200)
dragArea.TextTransparency = 0.2
dragArea.TextSize = isMinimized and 11 or 13
...
local function updateHitbox(character)
    ...
    if not hitboxCache[character] then
        ...
        if settings.Combat.Hitbox.Type == "Sphere" then
            ...
            box.Size = Vector3.new(settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size)
        end
        ...
        hrp:GetPropertyChangedSignal("Size"):Connect(function()
            if hitboxCache[character] then
                if settings.Combat.Hitbox.Type == "Sphere" then
                    hitboxCache[character].Radius = settings.Combat.Hitbox.Size
                else
                    hitboxCache[character].Size = Vector3.new(settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size)
                end
            end
        end)
    end
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        createToggle(scrollFrame, "Aim Assist", settings.Combat.AimAssist.Enabled, function(value)
            settings.Combat.AimAssist.Enabled = value
            if value then
                createAimAssistCircle()
            else
                clearAimAssist()
            end
        end)
        
        createValueChanger(scrollFrame, "Aim Assist Radius", {20, 30, 40, 50, 60, 70, 80}, 4, function(value)
            settings.Combat.AimAssist.Radius = value
        end)
