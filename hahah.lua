...
local configFile = "settings.silence"
...
local function saveConfig(configName)
    ensureConfigFolder()
    local configPath = configFolder.."/"..configName..".silence"
...
local function scanConfigs()
    ...
    for _, file in ipairs(files) do
        if file:sub(-8) == ".silence" and file ~= configFolder.."/"..configFile then
            local configName = file:match(".*/(.*)%.silence")
            if configName then
