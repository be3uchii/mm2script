...
local function getClosestPlayer()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            local targetPart = player.Character:FindFirstChild(settings.Combat.SilentAim.TargetPart)
            if targetPart then
...
local function updateSilentAim()
    ...
    if newTarget then
        if silentAimLastTarget ~= newTarget then
            silentAimLastTarget = newTarget
        end
        
        if settings.Combat.SilentAim.Smoothness > 0 then
...
local function refreshCombatUI()
    ...
    for i = 0, 100 do table.insert(smoothnessValues, i/100) end
    
    createValueChanger(combatScrollFrame, "Smoothness", smoothnessValues, settings.Combat.SilentAim.Smoothness, function(value)
        settings.Combat.SilentAim.Smoothness = value
        silentAimSmoothness = value
    end, "silentAimSmoothness")
end
        ...
        loadButton.MouseButton1Click:Connect(function()
            ...
            if loadConfig(configName) then
                for id, element in pairs(uiElements) do
                    ...
                    elseif id == "silentAimSmoothness" then
                        element.update(settings.Combat.SilentAim.Smoothness)
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if settings.Combat.SilentAim.Enabled and silentAimTarget and silentAimTarget.Character then
        if method:lower() == "fireserver" and tostring(self) == "RemoteEvent" then
            local targetPart = silentAimTarget.Character:FindFirstChild(settings.Combat.SilentAim.TargetPart)
            if targetPart then
                if silentAimSmoothness > 0 then
                    local currentPos = args[2]
                    local targetPos = targetPart.Position
                    local newPos = currentPos:Lerp(targetPos, silentAimSmoothness)
                    args[2] = newPos
                else
