...
local function updateESP(player)
    ...
    box.Visible = settings.ESP.Enabled and not settings.ESP.SkeletonOnly
    
    if settings.ESP.ShowDistance then
        distanceText.Text = tostring(math.floor(distance)) .. "m"
        distanceText.Position = Vector2.new(rootPos.X, headPos.Y - 15)
        distanceText.Visible = settings.ESP.Enabled and not settings.ESP.SkeletonOnly
    else
        distanceText.Visible = false
    end
...
local function refreshESPUI()
    ...
    createCleanToggle(espScrollFrame, "Skeleton Only", settings.ESP.SkeletonOnly, function(value)
        settings.ESP.SkeletonOnly = value
        updateAllESP()
    end, "skeletonOnly")
        ...
        loadButton.MouseButton1Click:Connect(function()
            ...
            if loadConfig(configName) then
                for id, element in pairs(uiElements) do
                    ...
                    elseif id == "skeletonOnly" then
                        element.update(settings.ESP.SkeletonOnly)
