...
local function CreateTracer(player)
    ...
    local function onCharacterAdded(character)
        ...
        playerConnections[player] = {
            ...
            health = humanoid:GetPropertyChangedSignal("Health"):Connect(function()
                if Tracers[player] then
                    Tracers[player].Visible = humanoid.Health > 0 and settings.ESP.Enabled and settings.ESP.ShowTracers
                end
            end)
        }
    end
    ...
    playerConnections[player.."charAdded"] = player.CharacterAdded:Connect(onCharacterAdded)
end
...
local function UpdateTracers()
    if not settings.ESP.Enabled or not settings.ESP.ShowTracers then return end
    
    ESP.Cache.ViewportSize = workspace.CurrentCamera.ViewportSize
    ESP.Cache.CameraCFrame = workspace.CurrentCamera.CFrame
