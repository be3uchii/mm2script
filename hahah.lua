...
local function ensureConfigFolder()
    ...
    if not isfile(configFolder.."/"..configFile) then
        writefile(configFolder.."/"..configFile, HttpService:JSONEncode(defaultSettings))
    end
end

local function saveSettings()
    ensureConfigFolder()
    settings.PlayerInfo.UserId = LocalPlayer.UserId
    settings.PlayerInfo.Name = LocalPlayer.Name
    settings.PlayerInfo.AccountAge = LocalPlayer.AccountAge
...
local function saveConfig(configName)
    ...
    local configData = table.clone(settings)
    configData.Configs = nil
...
local function loadConfig(configName)
    ...
    if isfile(configPath) then
        ...
        if success and result then
            for category, data in pairs(result) do
                if settings[category] and category ~= "PlayerInfo" and category ~= "Configs" then
                    for key, value in pairs(data) do
                        if settings[category][key] ~= nil then
                            settings[category][key] = value
                        end
