...
local function createSkeleton(player)
    ...
    local function onCharacterAdded(character)
        ...
        playerConnections[player].skeletonDied = humanoid.Died:Connect(function()
            for _, line in pairs(skeletons[player]) do
                line.Visible = false
            end
        end)
        
        playerConnections[player].skeletonRemoving = character.AncestryChanged:Connect(function(_, parent)
            if not parent then
                for _, line in pairs(skeletons[player]) do
                    line.Visible = false
                end
    ...
    playerConnections[player].skeletonAdded = player.CharacterAdded:Connect(onCharacterAdded)
end

local function updateSkeleton(player)
    if not settings.Skeleton.Enabled or not skeletons[player] then return end
    ...
    local distance = (character:FindFirstChild("HumanoidRootPart").Position - workspace.CurrentCamera.CFrame.Position).Magnitude
    if distance > settings.ESP.MaxDistance then
        for _, line in pairs(skeletons[player]) do
            line.Visible = false
        end
    ...
    for _, connection in ipairs(skeletonConnections) do
        ...
        local line = skeletons[player][connection[1]..connection[2]]
...
local function removeSkeleton(player)
    ...
    if playerConnections[player] then
        if playerConnections[player].skeletonDied then
            playerConnections[player].skeletonDied:Disconnect()
        end
        if playerConnections[player].skeletonAdded then
            playerConnections[player].skeletonAdded:Disconnect()
        end
        if playerConnections[player].skeletonRemoving then
            playerConnections[player].skeletonRemoving:Disconnect()
        end
    end
end
...
local function refreshESPUI()
    ...
    createCleanToggle(espScrollFrame, "Skeleton Enabled", settings.Skeleton.Enabled, function(value)
