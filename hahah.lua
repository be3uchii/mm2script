...
local function clearAll()
    ...
    for _, tracer in pairs(Tracers.Players) do
        if tracer then
...
local function createTracer(player)
    ...
    if Tracers.Players[player] then
        pcall(function() Tracers.Players[player]:Remove() end)
    end
    ...
    Tracers.Players[player] = tracer
end

local function updateTracer(player)
    local tracer = Tracers.Players[player]
    ...
    tracer.From = Vector2.new(Tracers.Cache.ViewportSize.X / 2, Tracers.Cache.ViewportSize.Y)
...
local function createESP(player)
    ...
    local function onCharacterAdded(character)
        ...
        playerConnections[player].died = humanoid.Died:Connect(function()
            ...
            if Tracers.Players[player] then
                Tracers.Players[player].Visible = false
            end
            if hitboxCache[character] then
                hitboxCache[character]:Destroy()
                hitboxCache[character] = nil
