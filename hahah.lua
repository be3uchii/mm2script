...
local function refreshMusicUI(scrollFrame)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if sliderDragging and input.UserInputType == Enum.UserInputType.MouseMovement and musicSoundInstance then
    ...
    local musicSoundConnection
    musicSoundConnection = RunService.Heartbeat:Connect(function()
        if musicSoundInstance and isMusicPlaying then
            updateMusicSlider()
        end
    end)

    table.insert(connections, musicSoundConnection)

    local endedConnection = musicSoundInstance.Ended:Connect(function()
        playTrack(currentTrack + 1)
    end)
