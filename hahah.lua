...
local function createColorButton(parent, text, value, callback, id)
    ...
    colorButton.MouseButton1Click:Connect(function()
        playSound()
        local currentIndex = table.find(combatColors, value) or 1
        currentIndex = currentIndex % #combatColors + 1
        value = combatColors[currentIndex]
        colorButton.BackgroundColor3 = value
        glow.Color = value
        callback(value)
        saveSettings()
...
local function createPasswordGUI()
    ...
    passwordBox.Position = UDim2.new(0.1, 0, 0, 85)
    ...
    local function restoreAttempts()
        while remainingAttempts < maxAttempts do
            task.wait(attemptCooldown)
            if remainingAttempts < maxAttempts then
