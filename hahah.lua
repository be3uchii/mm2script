...
local function updateHitbox(character)
    ...
    if not hitboxCache[character] then
        ...
        if settings.Combat.Hitbox.Type == "Sphere" then
            ...
            box.Radius = settings.Combat.Hitbox.Size * settings.Combat.Hitbox.Multiplier
        else
            box = Instance.new("BoxHandleAdornment")
            box.Size = Vector3.new(
                settings.Combat.Hitbox.Size * settings.Combat.Hitbox.Multiplier,
                settings.Combat.Hitbox.Size * settings.Combat.Hitbox.Multiplier,
                settings.Combat.Hitbox.Size * settings.Combat.Hitbox.Multiplier
            )
        end
        ...
        hrp:GetPropertyChangedSignal("Size"):Connect(function()
            if hitboxCache[character] then
                if settings.Combat.Hitbox.Type == "Sphere" then
                    hitboxCache[character].Radius = settings.Combat.Hitbox.Size * settings.Combat.Hitbox.Multiplier
                else
