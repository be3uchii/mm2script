...
local musicPlayer = {
    tracks = {
        ...
        {id = "76463442561219", name = "Powfu Death Bed"},
        {id = "98364034458260", name = "i love you so"},
        {id = "81477881808390", name = "madkid - распять"},
        {id = "123744226884947", name = "All i wanted was you"},
...
local function saveSettings()
    ...
    if musicPlayer.sound and musicPlayer.isPlaying then
        settings.Music.CurrentTime = musicPlayer.sound.TimePosition
    end
...
local function refreshMusicUI()
    ...
    sliderBackground.Position = UDim2.new(0, 0, 0.5, -2)
    ...
    sliderButton.Position = UDim2.new(0, 0, 0.5, -4)
    ...
    local prevButton = Instance.new("ImageButton")
    prevButton.Size = UDim2.new(0, 18, 0, 18)
    prevButton.Position = UDim2.new(0.25, -9, 0, 8)
    prevButton.BackgroundTransparency = 1
    ...
    playPauseButton.Size = UDim2.new(0, 18, 0, 18)
    playPauseButton.Position = UDim2.new(0.5, -9, 0, 8)
    ...
    local nextButton = Instance.new("ImageButton")
    nextButton.Size = UDim2.new(0, 18, 0, 18)
    nextButton.Position = UDim2.new(0.75, -9, 0, 8)
    nextButton.BackgroundTransparency = 1
    ...
    local function updateSlider()
        ...
        sliderButton.Position = UDim2.new(progress, -4, 0.5, -4)
    ...
    local function playTrack(trackIndex, noFade)
        if trackIndex < 1 then trackIndex = #musicPlayer.tracks end
        if trackIndex > #musicPlayer.tracks then trackIndex = 1 end
        
        local oldSound = musicPlayer.sound
        local oldVolume = oldSound and oldSound.Volume or 0.5
        ...
        musicPlayer.sound.Volume = noFade and oldVolume or 0
        ...
        if musicPlayer.isPlaying then
            ...
            if not noFade then
                local fadeIn = TweenService:Create(musicPlayer.sound, TweenInfo.new(0.5), {Volume = oldVolume})
                fadeIn:Play()
            end
        end
        ...
        if oldSound then
            if not noFade then
                local fadeOut = TweenService:Create(oldSound, TweenInfo.new(0.5), {Volume = 0})
                fadeOut:Play()
    ...
    local function nextTrack()
        playTrack(musicPlayer.currentTrack + 1)
    end
    ...
    local function togglePlayPause()
        ...
        if musicPlayer.sound then
            ...
            playTrack(musicPlayer.currentTrack, true)
        end
    ...
    prevButton.MouseButton1Click:Connect(function()
        if musicPlayer.sliderDebounce then return end
        musicPlayer.sliderDebounce = true
        playSound()
        prevTrack()
        task.wait(0.3)
        musicPlayer.sliderDebounce = false
    end)
    ...
    if connections.MusicEnded then
        connections.MusicEnded:Disconnect()
    end
    
    connections.MusicEnded = (musicPlayer.sound and musicPlayer.sound.Ended:Connect(nextTrack)) or nil
    
    if not musicPlayer.sound then
        playTrack(musicPlayer.currentTrack, true)
        if not musicPlayer.isPlaying and musicPlayer.sound then
            musicPlayer.sound:Pause()
        end
    else
        if musicPlayer.isPlaying and not musicPlayer.sound.IsPlaying then
            musicPlayer.sound:Play()
        elseif not musicPlayer.isPlaying and musicPlayer.sound.IsPlaying then
            musicPlayer.sound:Pause()
        end
        updateSlider()
    end
    ...
    RunService.Heartbeat:Connect(function()
        if musicPlayer.sound then
            ...
            if musicPlayer.sound.IsPlaying and musicPlayer.sound.TimePosition >= musicPlayer.sound.TimeLength - 0.1 then
                nextTrack()
            end
        end
