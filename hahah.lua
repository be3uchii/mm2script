...
local function createPasswordGUI()
    ...
    timerLabel.Text = "Генерация доступна"
    ...
    local function generateNewPassword()
        if not canGenerate then
            errorLabel.Text = string.format("Подождите %02d:%02d", math.floor(timer / 60), math.floor(timer % 60))
        ...
        generatedPassword = generatePassword(22)
    ...
    passwordBox.FocusLost:Connect(function(enterPressed)
        if enterPressed and checkPassword() then
            createMainGUI()
        end
    end)
...
local function init()
    ...
    game:BindToClose(function()
        ...
        if PlayerGui:FindFirstChild("UltimateFollowIcon") then
            PlayerGui.UltimateFollowIcon:Destroy()
        end
