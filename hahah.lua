...
local function loadSettings()
    ...
    if isfile(path) then
        ...
        if success and result then
            ...
            if result.Music then settings.Music = result.Music end
        end
    end
end
...
local function initializeMusicPlayer()
    ...
    musicPlayer.connection = game:GetService("RunService").Heartbeat:Connect(function()
        if musicPlayer.isPlaying then
            ...
            if uiElements.musicProgress then
                uiElements.musicProgress.update(progress)
            end
            if uiElements.musicTime then
                uiElements.musicTime.update(string.format("%d:%02d", math.floor(currentTime/60), math.floor(currentTime%60)).." / "..string.format("%d:%02d", math.floor(musicPlayer.soundLength/60), math.floor(musicPlayer.soundLength%60)))
            end
    ...
    musicPlayer.sound.Loaded:Connect(function()
        ...
        if uiElements.musicTime then
            uiElements.musicTime.update("0:00 / "..string.format("%d:%02d", math.floor(musicPlayer.soundLength/60), math.floor(musicPlayer.soundLength%60)))
        end
    end)
end
...
local function refreshMusicUI()
    ...
    playButton.Position = UDim2.new(0.5, -80, 0, 10)
    playButton.Image = "rbxassetid://12099513379"
    playButton.BackgroundTransparency = 1
    playButton.Parent = musicScrollFrame
    
    local pauseButton = Instance.new("ImageButton")
    pauseButton.Name = "PauseButton"
    pauseButton.Size = UDim2.new(0, 32, 0, 32)
    pauseButton.Position = UDim2.new(0.5, -80, 0, 10)
    pauseButton.Image = "rbxassetid://14219414360"
    pauseButton.BackgroundTransparency = 1
    pauseButton.Visible = false
    pauseButton.Parent = musicScrollFrame
    ...
    fillUICorner.Parent = progressFill
    ...
    progressHandle.Parent = progressFill
    ...
    prevButton.Size = UDim2.new(0, 28, 0, 28)
    prevButton.Position = UDim2.new(0.5, -40, 0, 10)
    prevButton.Image = "rbxassetid://12662712394"
    prevButton.BackgroundTransparency = 1
    prevButton.Parent = musicScrollFrame
    
    local nextButton = Instance.new("ImageButton")
    nextButton.Name = "NextButton"
    nextButton.Size = UDim2.new(0, 28, 0, 28)
    nextButton.Position = UDim2.new(0.5, 10, 0, 10)
    nextButton.Image = "rbxassetid://12662720464"
    nextButton.BackgroundTransparency = 1
    nextButton.Parent = musicScrollFrame
    
    local timeText = Instance.new("TextLabel")
    timeText.Name = "TimeText"
    timeText.Size = UDim2.new(1, -20, 0, 16)
    timeText.Position = UDim2.new(0, 10, 0, 70)
    timeText.BackgroundTransparency = 1
    timeText.TextColor3 = Color3.fromRGB(230, 230, 230)
    ...
    timeText.Parent = musicScrollFrame
    
    uiElements.musicProgress = {
        update = function(progress)
            progressFill.Size = UDim2.new(progress, 0, 1, 0)
        end
    }
    
    uiElements.musicTime = {
    ...
    local volumeValues = {}
    for i = 0, 10 do table.insert(volumeValues, i/10) end
