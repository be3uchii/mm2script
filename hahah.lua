...
local function updateHitbox(character)
    ...
    if box then
        if settings.Combat.Hitbox.Type == "Sphere" then
            box.Radius = settings.Combat.Hitbox.Size
            box.Adornee.Size = Vector3.new(settings.Combat.Hitbox.Size * 2, settings.Combat.Hitbox.Size * 2, settings.Combat.Hitbox.Size * 2)
        else
            box.Size = Vector3.new(settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size * 1.2, settings.Combat.Hitbox.Size)
            box.Adornee.Size = Vector3.new(settings.Combat.Hitbox.Size, settings.Combat.Hitbox.Size * 1.2, settings.Combat.Hitbox.Size)
        end
        box.Transparency = settings.Combat.Hitbox.Transparency
        box.Color3 = settings.Combat.Hitbox.Color
        box.AlwaysOnTop = settings.Combat.Hitbox.ThroughWalls
...
local function clearHitboxes()
    for character, box in pairs(hitboxCache) do
        ...
        box:Destroy()
        if character:FindFirstChild("HitboxCollision") then
            character.HitboxCollision:Destroy()
        end
    end
    hitboxCache = {}
end
...
local function updateSpeed()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
        ...
        humanoid.WalkSpeed = settings.Misc.Speed.Enabled and settings.Misc.Speed.Value or defaultWalkSpeed
    end
end

local function updateInfiniteJump()
    if settings.Misc.InfiniteJump.Enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
        local humanoid = LocalPlayer.Character.Humanoid
...
for i, tabName in ipairs(tabs) do
    ...
    elseif tabName == "Misc" then
        ...
        local speedValues = {16, 17, 18, 19}
        createValueChanger(scrollFrame, "Speed Value", speedValues, 1, function(value)
