    ...
    if isfile(path) then
        safeCall(function()
            ...
            if success and result then
                ...
                if result.Skeletons then
                    settings.Skeletons = result.Skeletons
                end
            end
        end, "loadSettings")
    end
...
local function createSkeleton(player)
    ...
    for _, connection in ipairs(skeletonConnections) do
        ...
        line.Transparency = settings.Skeletons.Transparency
        line.ZIndex = 1
    ...
    local function onCharacterAdded(character)
        ...
        skeletonConnections[player] = humanoid.Died:Connect(function()
...
local function updateSkeletons()
    if not settings.Skeletons.Enabled then
        for _, skeleton in pairs(skeletons) do
            for _, line in pairs(skeleton) do
                line.Visible = false
            end
    ...
    for player, skeleton in pairs(skeletons) do
        if player and player ~= LocalPlayer and player.Character then
            ...
            if character and humanoid and humanoid.Health > 0 then
                local distance = (character:GetPivot().Position - camera.CFrame.Position).Magnitude
...
local function refreshESPUI()
    ...
    createCleanToggle(espScrollFrame, "Show Skeletons", settings.Skeletons.Enabled, function(value)
        settings.Skeletons.Enabled = value
        if not value then
            for _, skeleton in pairs(skeletons) do
                for _, line in pairs(skeleton) do
                    line.Visible = false
    ...
    local thicknessValues = {1, 2, 3, 4, 5}
    createValueChanger(espScrollFrame, "Skeleton Thickness", thicknessValues, settings.Skeletons.Thickness, function(value)
        settings.Skeletons.Thickness = value
        updateAllESP()
    end, "skeletonThickness")
    
    createColorButton(espScrollFrame, "Skeleton Color", settings.Skeletons.Color, function(value)
        settings.Skeletons.Color = value
        updateAllESP()
    end, "skeletonColor")
    
    createValueChanger(espScrollFrame, "Skeleton Transparency", transparencies, settings.Skeletons.Transparency, function(value)
        settings.Skeletons.Transparency = value
        updateAllESP()
    end, "skeletonTransparency")
end
        ...
        loadButton.MouseButton1Click:Connect(function()
            ...
            if loadConfig(configName) then
                for id, element in pairs(uiElements) do
                    ...
                    elseif id == "skeletonsEnabled" then
                        element.update(settings.Skeletons.Enabled)
                    elseif id == "espMaxDistance" then
                        element.update(settings.ESP.MaxDistance)
                    elseif id == "skeletonThickness" then
                        element.update(settings.Skeletons.Thickness)
                    elseif id == "skeletonColor" then
                        element.update(settings.Skeletons.Color)
                    elseif id == "skeletonTransparency" then
                        element.update(settings.Skeletons.Transparency)
