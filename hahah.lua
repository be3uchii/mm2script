...
local function createPasswordGUI()
    ...
    timerLabel.Text = "До генерации пароля: 60 сек"
    ...
    local function updateTimerDisplay()
        timerLabel.Text = "До генерации пароля: " .. timer .. " сек"
    end
    ...
    local function startTimer()
        ...
        timerConnection = RunService.Heartbeat:Connect(function(delta)
            ...
            if timer <= 0 then
                timer = 0
                canGenerate = true
                timerLabel.Text = "Можно сгенерировать пароль"
                timerConnection:Disconnect()
            else
                updateTimerDisplay()
            end
    ...
    local function checkPassword()
        ...
        if password == generatedPassword and generatedPassword ~= "" then
            playSound()
            ensureConfigFolder()
            local expireTime = os.time() + 86400 -- 24 часа
            local data = {
