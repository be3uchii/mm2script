...
local menuWidth, menuHeight = 280, 150
...
local minimizedSize = UDim2.new(0, 70, 0, 22)
...
local defaultSettings = {
    PlayerInfo = {
        ...
        DeviceType = UserInputService.TouchEnabled and "Mobile" or "PC",
        JoinTime = os.time(),
...
local function saveSettings()
    ensureConfigFolder()
    settings.PlayerInfo.LastSessionDuration = os.time() - settings.PlayerInfo.ScriptStartTime
    ...
    if isfile(path) then
        ...
        if success and result and result.Configs then
            ...
            if result.PlayerInfo then
                settings.PlayerInfo.LastSessionDuration = result.PlayerInfo.LastSessionDuration or 0
                settings.PlayerInfo.LaunchCount = (result.PlayerInfo.LaunchCount or 0) + 1
                settings.PlayerInfo.ScriptStartTime = os.time()
            end
        end
    end
end
...
local function createCleanToggle(parent, text, value, callback, id)
    ...
    ToggleCircle.Position = value and UDim2.new(1, -16, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)
    ...
    SwitchButton.MouseButton1Click:Connect(function()
        ...
        local circleGoalPos = value and UDim2.new(1, -16, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)
...
local function createMainGUI()
    ...
    for i, tabName in ipairs(tabs) do
        ...
        tabIcon.Size = i == 1 and UDim2.new(0, 27, 0, 27) or UDim2.new(0, 18, 0, 18)
        tabIcon.Position = i == 1 and UDim2.new(0.5, -13.5, 0.5, -13.5) or UDim2.new(0.5, -9, 0.5, -9)
        ...
        contentFrame.Position = UDim2.new(0, 4, 0, 48)
