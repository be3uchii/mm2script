...
local settings = {
    Combat = {
        Hitbox = {
            ...
            Transparency = 0.7,
            ThroughWalls = false,
            DamageParts = {"Head", "UpperTorso", "LowerTorso", "LeftUpperArm", "RightUpperArm", "LeftUpperLeg", "RightUpperLeg"}
        }
    },
    ESP = {
        Enabled = false,
    ...
    local function onCharacterAdded(character)
        ...
        playerConnections[player].died = humanoid.Died:Connect(function()
            ...
            if hitboxCache[character] then
                for _, part in pairs(hitboxCache[character]) do
                    part:Destroy()
                end
...
local function updateHitbox(character)
    ...
    if not hitboxCache[character] then
        ...
        for _, partName in ipairs(settings.Combat.Hitbox.DamageParts) do
            local realPart = character:FindFirstChild(partName)
            if realPart then
                local box = Instance.new("BoxHandleAdornment")
                box.Name = "HitboxAdornment"
                box.Adornee = realPart
                ...
                box.Size = realPart.Size * 1.2
                box.Parent = workspace
                table.insert(hitboxCache[character], box)
        ...
        for _, box in ipairs(hitboxCache[character]) do
...
local function clearHitboxes()
    for character, boxes in pairs(hitboxCache) do
