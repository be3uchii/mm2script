...
local function showWarning(message, duration)
    ...
    warningLabel.Size = UDim2.new(1, -20, 0.7, -10)
    ...
    okButton.Size = UDim2.new(0.6, 0, 0.2, 0)
    okButton.Position = UDim2.new(0.2, 0, 0.75, 0)
    okButton.BackgroundColor3 = Color3.fromRGB(60, 60, 80)
    okButton.Text = "OK ("..duration..")"
...
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    local function updateValue()
        ...
        if text == "Hitbox Size" and values[currentIndex] > 8 then
            showWarning("Large hitbox sizes may be detectable!", 10)
        end
    end
...
local function updateChams(character)
    ...
    if chamsCache[character] then
        for _, part in pairs(chamsCache[character]) do
            ...
            part.Color3 = settings.ESP.Color
        end
    end
end
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        local sizeValues = {}
        for i = 1, 30 do table.insert(sizeValues, i) end
