...
local function updateESP(player)
    ...
    if distance > settings.ESP.MaxDistance then return false end
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        createValueChanger(scrollFrame, "Размер Хитбокса", {4, 6, 8, 10, 12, 14, 16, 18}, math.floor(settings.Combat.Hitbox.Size / 2), function(value)
            settings.Combat.Hitbox.Size = value * 2
            updateAllESP()
        end)
    ...
    elseif tabName == "ESP" then
        ...
        createValueChanger(scrollFrame, "Макс. дистанция", {100, 150, 200, 250, 300, 350, 400}, math.floor(settings.ESP.MaxDistance / 50), function(value)
            settings.ESP.MaxDistance = value * 50
            updateAllESP()
        end)
    elseif tabName == "Misc" then
        ...
        createValueChanger(scrollFrame, "Значение Скорости", {16, 18, 20, 22, 24, 26, 28, 30}, math.floor((settings.Misc.Speed.Value - 16) / 2) + 1, function(value)
            settings.Misc.Speed.Value = 16 + (value - 1) * 2
            updateSpeed()
        end)
    end
