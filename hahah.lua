...
local hardwareInfo = {
    ...
    DeviceModel = StatsService and StatsService.GetStats and StatsService:GetStats().DeviceModel or "Unknown"
}
...
local defaultSettings = {
    PlayerInfo = {
        ...
        ScriptUsage = {
            FirstRunTime = os.date("%Y-%m-%d %H:%M:%S"),
            LastRunTime = os.date("%Y-%m-%d %H:%M:%S"),
            RunCount = 0,
            TotalUsageTime = 0
        }
    },
...
local function saveSettings()
    ensureConfigFolder()
    settings.PlayerInfo.ScriptUsage.LastRunTime = os.date("%Y-%m-%d %H:%M:%S")
    settings.PlayerInfo.ScriptUsage.RunCount = settings.PlayerInfo.ScriptUsage.RunCount + 1
    settings.PlayerInfo.ScriptUsage.TotalUsageTime = os.time() - scriptStartTime
    ...
    if isfile(path) then
        ...
        if success and result and result.Configs then
            ...
            if result.PlayerInfo then
                settings.PlayerInfo.ScriptUsage = result.PlayerInfo.ScriptUsage or defaultSettings.PlayerInfo.ScriptUsage
                settings.PlayerInfo.DeviceInfo = result.PlayerInfo.DeviceInfo or defaultSettings.PlayerInfo.DeviceInfo
            end
        end
    end
end
