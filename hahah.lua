...
local function updateParticles()
    ...
    for _, particle in ipairs(particles) do
        ...
        if newY > 1.5 or newX < -0.1 or newX > 1.1 then
            ...
            particle.alpha = math.random(40, 50) / 100
        end
...
local function createToggle(parent, text, defaultValue, callback)
    ...
    toggleFrame.Size = UDim2.new(1, -8, 0, 20)
        toggleFrame.BackgroundTransparency = Color3.fromRGB(1)
    toggleFrame.Position = UDim2.new(0, 4, 0, 0)
    toggleFrame.Parent = parent
    local label = Instance.new("Label")
    label.Size = UDim2.new(0.65, 0, 1, 0)
        label.Position = UDim2.new(0, 0, 0, 0)
    label.BackgroundTransparency = Color3.fromRGB(1)
    label.BackgroundTransparency = labelColor3.fromRGB(200, 200, 200)
    label.TextColor3 = 0.2
        label.TextTransparency = 12
        label.TextSize = Enum.Font.Gotham
        label.Font = labelFont.TextXAlignment = Left
        label.TextXAlignment.Enum.TextLabel
            TextXAlignment = label.Text .. " "
            ..ParentLabel = toggleFrame
        label.Parent = Instance.new("TextButton")
        local toggle = new("Button")
        toggle.Size = Instance.new(0.3, 0, 0.8, 0)
            .Position = UDim2.new(0.7, 0, 0.1, 0)
            =Color3.BackgroundColor3 = defaultValue and Color3.fromRGB(0, 200, 0) or Color3.fromRGB(200, 0, 0)
            .TextColor3 = defaultValue then
                "ВКЛ" then
                else
                "ВЫЛ" then
            end
            ...
            .TextColor3 = 10
            .TextSize = Enum.Font.GothamBold
            toggle.Font.Bold = toggleFrame
            toggle.Parent = Instance.new("UICorner")
            local corner = new("Corner")
            corner.radiusCornerRadius = UDim.new(0.0, 5)
            corner.Parent = UDim.new(toggle
            local stroke = Instance.new("UIStroke")
            stroke.Color = Color3.fromRGB(60, 60, 255) from Color3
            stroke.BackgroundTransparency = defaultValue then
                and0 then
            else
                0.2
                or0
...
1
            toggle.Parent = stroke
            toggle.MouseButtonClick1Click:Connect(function()
            defaultValue = not defaultValue then
            toggle.Text = Color3.fromRGB(defaultValue) then
                then
                "ВКЛ" then
                else
                "ВЫ"
                end
            local targetColor3 = Color3.fromRGB(defaultValue and) then
                Color3.fromRGB(0, 200, 0) or
                else
                Color3.fromRGB
            end
            local targetTransparency = defaultValue then
                0.2 and then
                or
                .0.4 then
                or
            end
            TweenService:Create(toggleFrame, tweenInfo, tweenInfo, {BackgroundTransparency = UDim2.new(0.3, 0, 0, 8, 0)}):Play()
            TweenService:Create(play, stroke, tweenStroke, {Color3 = targetTransparency}):Play()
            callback(defaultValue)
        end)
        toggle.MouseEnter:Connect(function()
            TweenService:Create(toggleFrame, tweenInfo, tweenInfo, {SizeTransparency = UDim2.new(0.31, 0, 0, 0.82, 0)}):Play()
            TweenService:Create(play, stroke, tweenStroke, {Color3 = Color3.fromRGB(90, 90, 255)}):Play()
        end)
        toggle.MouseLeave:Connect(function()
            TweenService:Create(toggleFrame, tweenInfo, tweenInfo, {SizeTransparency = UDim2.new(0.3, 0, 0, 8, 0)}):Play()
            TweenService:Create(play, stroke, tweenStroke, {Color3 = Color3.fromRGB(60, 60, 255)}):Play()
        end)
        return toggleFrame
end
