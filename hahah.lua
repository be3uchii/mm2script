                ...
                if settings[category] then
                    for key, value in pairs(data) do
                        if settings[category][key] ~= nil then
                            if category == "Combat" and key == "Hitbox" and value.Color then
...
local function saveConfig(configName)
    ...
    local configData = deepCopy(settings)
...
local function loadConfig(configName)
    ...
    if isfile(configPath) then
        ...
        if success and result then
            ...
            warn("Failed to load config: "..(result or "unknown error"))
        end
    end
    return false
end
...
local function refreshConfigUI(scrollFrame)
    ...
    createButton.MouseButton1Click:Connect(function()
        if textBox.Text ~= "" then
            if saveConfig(textBox.Text) then
