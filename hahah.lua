...
local function createToggle(parent, text, settingPath, callback)
    ...
    uiElements[text] = {
        Update = function(value)
            settings[settingPath[1]][settingPath[2]] = value
...
local function createValueChanger(parent, text, values, settingPath, callback)
    ...
    uiElements[text] = {
        Update = function(value)
            currentIndex = table.find(values, value) or 1
            settings[settingPath[1]][settingPath[2]] = values[currentIndex]
            valueLabel.Text = tostring(settings[settingPath[1]][settingPath[2]])
        end
    }
...
local function createColorButton(parent, text, settingPath, callback)
    ...
    colorButton.MouseButton1Click:Connect(function()
        local currentIndex = table.find(colors, settings[settingPath[1]][settingPath[2]]) or 1
...
local function refreshCombatUI()
    ...
    createToggle(combatScrollFrame, "Hitbox Enabled", {"Combat", "Hitbox", "Enabled"}, function(value)
        if not value then
            clearHitboxes()
        end
        updateAllESP()
    ...
    createValueChanger(combatScrollFrame, "Hitbox Size", sizeValues, {"Combat", "Hitbox", "Size"}, updateAllESP)
    
    createToggle(combatScrollFrame, "Sphere Hitbox", {"Combat", "Hitbox", "Type"}, function(value)
        settings.Combat.Hitbox.Type = value and "Sphere" or "Box"
        clearHitboxes()
        updateAllESP()
    end)
    
    createToggle(combatScrollFrame, "Through Walls", {"Combat", "Hitbox", "ThroughWalls"}, updateAllESP)
    
    createColorButton(combatScrollFrame, "Hitbox Color", {"Combat", "Hitbox", "Color"}, updateAllESP)
    
    createValueChanger(combatScrollFrame, "Hitbox Transparency", transparencies, {"Combat", "Hitbox", "Transparency"}, updateAllESP)
end
...
local function refreshESPUI()
    ...
    createToggle(espScrollFrame, "ESP Enabled", {"ESP", "Enabled"}, updateAllESP)
    createToggle(espScrollFrame, "Show Distance", {"ESP", "ShowDistance"}, updateAllESP)
end
