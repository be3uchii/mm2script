...
local function createESP(player)
    ...
    local function onCharacterAdded(character)
        ...
        playerConnections[player].added = character.ChildAdded:Connect(function(child)
            if child:IsA("BasePart") and chamsCache[character] and settings.ESP.Chams then
                local box = Instance.new("BoxHandleAdornment")
                box.Adornee = child
                box.Size = child:GetMass() > 1 and child.Size-Vector3.new(0.1,0.1,0.1) or Vector3.new(0.4,0.4,0.4)
...
local function createChams(character)
    ...
    for _, part in pairs(character:GetDescendants()) do
        if part:IsA("BasePart") and not part:FindFirstChild("HitboxAdornment") then
            local box = Instance.new("BoxHandleAdornment")
            box.Adornee = part
    ...
    for _, target in ipairs(Players:GetPlayers()) do
        if target ~= LocalPlayer and target.Character then
            ...
            if targetRoot then
                ...
                if distance <= settings.Combat.KillAll.Distance and not frozenPlayers[targetChar] then
                ...
                if player.Character then
                    ...
                    if settings.Combat.KillAll.Enabled then
                        ...
                        if root and player.Character then
                            ...
                            if targetRoot then
                                local distance = (targetRoot.Position - root.Position).Magnitude
                                if distance <= settings.Combat.KillAll.Distance and not frozenPlayers[player.Character] then
                                    targetRoot.CFrame = root.CFrame * CFrame.new(math.random(-4, 4), 0, math.random(-4, 4))
