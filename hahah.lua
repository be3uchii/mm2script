...
local function playSound(id, volume, pitch)
    if not id then
        if not soundCache.clickSound then
            ...
            soundCache.clickSound.SoundId = "rbxassetid://357129686"
            soundCache.clickSound.Volume = volume or 0.25
            soundCache.clickSound.Pitch = pitch or 1
        ...
        if not soundCache[id] then
            ...
            soundCache[id].Pitch = pitch or 1
...
local function clearAll()
    ...
    for _, tween in pairs(pulseTweens) do
        pcall(function() tween:Cancel() end)
    end
    pulseTweens = {}
    
    for _, effect in pairs(glowEffects) do
...
local function createCleanToggle(parent, text, value, callback, id)
    ...
    SwitchButton.Size = UDim2.new(0.25, 0, 0.7, 0)
    SwitchButton.Position = UDim2.new(0.7, 0, 0.15, 0)
    ...
    hoverGlow.Thickness = 1
    hoverGlow.Transparency = 1
    hoverGlow.Parent = SwitchButton
    
    SwitchButton.MouseEnter:Connect(function()
        TweenService:Create(hoverGlow, tweenInfo, {Transparency = 0.7}):Play()
    end)
    
    SwitchButton.MouseLeave:Connect(function()
        TweenService:Create(hoverGlow, tweenInfo, {Transparency = 1}):Play()
    end)
    
    SwitchButton.MouseButton1Click:Connect(function()
        playSound(nil, 0.3, 1.2)
        value = not value
        
        local circleGoalPos = value and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7)
...
local function createValueChanger(parent, text, values, value, callback, id)
    ...
    prevButton.ImageTransparency = 0.2
    ...
    nextButton.ImageTransparency = 0.2
...
local function createColorButton(parent, text, value, callback, id)
    ...
    colorButton.MouseButton1Click:Connect(function()
        playSound(nil, 0.3, 1.1)
        local currentIndex = table.find(colors, value) or 1
        currentIndex = currentIndex % #colors + 1
        value = colors[currentIndex]
        TweenService:Create(colorButton, tweenInfo, {BackgroundColor3 = value}):Play()
        TweenService:Create(glow, tweenInfo, {Color = value}):Play()
        callback(value)
        saveSettings()
    end)
...
local function refreshConfigUI(scrollFrame)
    ...
    createConfigFrame.Size = UDim2.new(1, -8, 0, 55)
    ...
    createButton.TextColor3 = Color3.fromRGB(240, 240, 240)
    ...
    for _, configName in ipairs(scanConfigs()) do
        ...
        configFrame.Size = UDim2.new(1, -8, 0, 45)
        ...
        loadButton.TextColor3 = Color3.fromRGB(240, 240, 240)
        ...
        deleteButton.TextColor3 = Color3.fromRGB(240, 240, 240)
...
local function createKeyGUI()
    ...
    attemptsLabel.Position = UDim2.new(0, 10, 0, 65)
    ...
    generateButton.Size = UDim2.new(0.35, 0, 0, 35)
    generateButton.Position = UDim2.new(0.1, 0, 0, 135)
    generateButton.BackgroundColor3 = Color3.fromRGB(60, 60, 80)
    generateButton.TextColor3 = Color3.fromRGB(240, 240, 240)
    ...
    submitButton.Position = UDim2.new(0.55, 0, 0, 135)
...
local function createMainGUI()
    ...
    rightIcon.Size = UDim2.new(0, 22, 0, 22)
    rightIcon.Position = UDim2.new(0.5, 40, 0.5, -11)
    ...
    for i, tabName in ipairs(tabs) do
        ...
        tabIcon.Size = i == 1 and UDim2.new(0, 22, 0, 22) or UDim2.new(0, 18, 0, 18)
        tabIcon.Position = i == 1 and UDim2.new(0.5, -11, 0.5, -11) or UDim2.new(0.5, -9, 0.5, -9)
