...
local musicTracks = {
    {id = 142376088, name = "Raining Tacos"},
...
local function createSlider(parent, text, minValue, maxValue, currentValue, callback, id)
    ...
    thumb.Position = UDim2.new((currentValue - minValue) / (maxValue - minValue), -5, 0.5, -5)
    ...
    local function updateValue(input)
        ...
        thumb.Position = UDim2.new(relativeX, -5, 0.5, -5)
        valueLabel.Text = tostring(value)
        callback(value)
    end
...
local function refreshMusicUI()
    ...
    local function playCurrentTrack()
        ...
        currentMusicSound.SoundId = "rbxassetid://"..musicTracks[settings.Music.CurrentTrack].id
        currentMusicSound.Volume = settings.Music.Volume
        currentMusicSound.Looped = false
        currentMusicSound.Parent = workspace
        
        if settings.Music.IsPlaying then
            currentMusicSound:Play()
        end
        
        currentMusicSound.Ended:Connect(function()
            if settings.Music.IsPlaying then
                settings.Music.CurrentTrack = settings.Music.CurrentTrack % #musicTracks + 1
                playCurrentTrack()
                if uiElements["musicTrack"] then
                    uiElements["musicTrack"].update(musicTracks[settings.Music.CurrentTrack].name)
                end
            end
        end)
    end
    ...
    trackLabel.Text = "Track: "..musicTracks[settings.Music.CurrentTrack].name
    ...
    playPauseButton.ImageColor3 = Color3.fromRGB(200, 200, 200)
    ...
    nextButton.BackgroundTransparency = 1
    nextButton.Image = "rbxassetid://13321918705"
    nextButton.ImageColor3 = Color3.fromRGB(200, 200, 200)
    nextButton.Parent = controlsFrame
    
    uiElements["playPauseButton"] = {
        update = function(isPlaying)
            playPauseButton.Image = isPlaying and "rbxassetid://14219414360" or "rbxassetid://8215093320"
        end
    }
    
    createSlider(musicScrollFrame, "Volume", 0, 1, settings.Music.Volume, function(value)
        settings.Music.Volume = value
        if currentMusicSound then
            currentMusicSound.Volume = value
        end
        saveSettings()
    end, "musicVolume")
    ...
    prevButton.MouseButton1Click:Connect(function()
        playSound()
        settings.Music.CurrentTrack = settings.Music.CurrentTrack > 1 and settings.Music.CurrentTrack - 1 or #musicTracks
        trackLabel.Text = "Track: "..musicTracks[settings.Music.CurrentTrack].name
        playCurrentTrack()
        saveSettings()
    end)
    ...
    playPauseButton.MouseButton1Click:Connect(function()
        ...
        uiElements["playPauseButton"].update(settings.Music.IsPlaying)
        saveSettings()
    end)
        ...
        loadButton.MouseButton1Click:Connect(function()
            ...
            if loadConfig(configName) then
                for id, element in pairs(uiElements) do
                    ...
                    elseif id == "musicTrack" then
                        element.update(musicTracks[settings.Music.CurrentTrack].name)
                    elseif id == "playPauseButton" then
                        element.update(settings.Music.IsPlaying)
