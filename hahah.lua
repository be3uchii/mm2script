...
local minimizedPosition = UDim2.new(0.5, -45, 0, 12)
local defaultPosition = UDim2.new(0, menuX, 0, menuY)
local warnedAboutHitbox = false
...
local function showWarning()
    if warnedAboutHitbox then return end
    
    local warningFrame = Instance.new("Frame")
    warningFrame.Size = UDim2.new(0.8, 0, 0, 100)
    warningFrame.Position = UDim2.new(0.1, 0, 0.5, -50)
    ...
    warningText.Size = UDim2.new(1, -10, 0.7, -10)
    warningText.Position = UDim2.new(0, 5, 0, 5)
    warningText.BackgroundTransparency = 1
    warningText.Text = "Внимание! Хитбоксы больше 8 могут привести к банам из-за жалоб игроков. Используйте с осторожностью!"
    ...
    countdownConnection = RunService.Heartbeat:Connect(function()
        ...
        if countdown <= 0 then
            okButton.Text = "OK"
            okButton.BackgroundColor3 = Color3.fromRGB(80, 80, 100)
            countdownConnection:Disconnect()
        else
            okButton.Text = "OK ("..countdown..")"
        end
    end)
...
local function createValueChanger(parent, text, values, defaultValueIndex, callback)
    ...
    local function updateValue()
        ...
        if text == "Hitbox Size" and values[currentIndex] > 8 and not warnedAboutHitbox then
            showWarning()
        end
    end
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        local sizeValues = {}
        for i = 1, 14 do table.insert(sizeValues, i) end
