...
local function CreateTracer(player)
    if Tracers.Players[player] or player == LocalPlayer then return end
    
    Tracers.Players[player] = true
    
    local tracer = Drawing.new("Line")
    tracer.Visible = false
    tracer.Color = Tracers.Settings.Color
    tracer.Thickness = 1
    tracer.ZIndex = 1
    
    Tracers.Tracers[player] = tracer

    local function onCharacterAdded(character)
        local humanoid = character:WaitForChild("Humanoid")
        
        Tracers.Connections[player] = humanoid.Died:Connect(function()
            tracer.Visible = false
        end)
        
        Tracers.Connections[player.."Health"] = humanoid:GetPropertyChangedSignal("Health"):Connect(function()
            tracer.Visible = humanoid.Health > 0 and Tracers.Settings.Enabled
        end)
    end
...
local function RemoveTracer(player)
    if Tracers.Tracers[player] then
        Tracers.Tracers[player]:Remove()
        Tracers.Tracers[player] = nil
    end

    if Tracers.Connections[player] then
        Tracers.Connections[player]:Disconnect()
        Tracers.Connections[player] = nil
    end
    
    if Tracers.Connections[player.."Health"] then
        Tracers.Connections[player.."Health"]:Disconnect()
        Tracers.Connections[player.."Health"] = nil
    end

    Tracers.Players[player] = nil
end

local function UpdateTracers()
    Tracers.Cache.ViewportSize = workspace.CurrentCamera.ViewportSize
    Tracers.Cache.CameraCFrame = workspace.CurrentCamera.CFrame
    local cameraPos = Tracers.Cache.CameraCFrame.Position
    local viewportCenter = Vector2.new(Tracers.Cache.ViewportSize.X/2, Tracers.Cache.ViewportSize.Y)

    for player, tracer in pairs(Tracers.Tracers) do
            ...
            if humanoidRootPart and humanoid and humanoid.Health > 0 then
                ...
                if distance <= settings.ESP.MaxDistance then
                    ...
                    if onScreen then
                        ...
                        tracer.Visible = settings.ESP.Enabled and settings.ESP.ShowTracers
                        continue
                    end
                end
            end
...
local function InitializeTracers()
    for _, player in ipairs(Players:GetPlayers()) do
        task.spawn(CreateTracer, player)
    end
...
local function clearAll()
    ...
    for _, conn in pairs(Tracers.Connections) do
        if conn then
            pcall(function() conn:Disconnect() end)
        end
    end
...
local function refreshESPUI()
    ...
    createCleanToggle(espScrollFrame, "Show Tracers", settings.ESP.ShowTracers, function(value)
        settings.ESP.ShowTracers = value
        Tracers.Settings.Enabled = value
        updateAllESP()
    end, "espShowTracers")
        ...
        loadButton.MouseButton1Click:Connect(function()
            ...
            if loadConfig(configName) then
                for id, element in pairs(uiElements) do
                    ...
                    elseif id == "espShowTracers" then
                        element.update(settings.ESP.ShowTracers)
