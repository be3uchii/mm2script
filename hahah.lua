...
local function createToggle(parent, text, value, callback)
    ...
    return toggleFrame, toggle, function(newValue)
        value = newValue
        toggle.Text = value and "ON" or "OFF"
        toggle.BackgroundColor3 = value and Color3.fromRGB(0, 230, 0) or Color3.fromRGB(230, 0, 0)
    end
...
local function createValueChanger(parent, text, values, value, callback)
    ...
    return changerFrame, valueLabel, function(newValue)
...
local function createColorButton(parent, text, value, callback)
    ...
    return colorFrame, colorButton, function(newValue)
        value = newValue
        colorButton.BackgroundColor3 = value
    end
end
...
local function refreshCombatUI()
    ...
    local hitboxToggleFrame, hitboxToggle, updateHitboxToggle = createToggle(combatScrollFrame, "Hitbox Enabled", settings.Combat.Hitbox.Enabled, function(value)
        settings.Combat.Hitbox.Enabled = value
        updateHitboxToggle(value)
        if not value then
            clearHitboxes()
        end
        updateAllESP()
    ...
    local sizeChangerFrame, sizeLabel, updateSizeLabel = createValueChanger(combatScrollFrame, "Hitbox Size", sizeValues, settings.Combat.Hitbox.Size, function(value)
        settings.Combat.Hitbox.Size = value
        updateSizeLabel(value)
        updateAllESP()
    end)
    
    local sphereToggleFrame, sphereToggle, updateSphereToggle = createToggle(combatScrollFrame, "Sphere Hitbox", settings.Combat.Hitbox.Type == "Sphere", function(value)
        settings.Combat.Hitbox.Type = value and "Sphere" or "Box"
        updateSphereToggle(value)
        clearHitboxes()
        updateAllESP()
    end)
    
    local wallsToggleFrame, wallsToggle, updateWallsToggle = createToggle(combatScrollFrame, "Through Walls", settings.Combat.Hitbox.ThroughWalls, function(value)
        settings.Combat.Hitbox.ThroughWalls = value
        updateWallsToggle(value)
        updateAllESP()
    end)
    
    local colorFrame, colorButton, updateColorButton = createColorButton(combatScrollFrame, "Hitbox Color", settings.Combat.Hitbox.Color, function(value)
        settings.Combat.Hitbox.Color = value
        updateColorButton(value)
        updateAllESP()
    end)
    
    local transChangerFrame, transLabel, updateTransLabel = createValueChanger(combatScrollFrame, "Hitbox Transparency", transparencies, settings.Combat.Hitbox.Transparency, function(value)
        settings.Combat.Hitbox.Transparency = value
        updateTransLabel(value)
        updateAllESP()
    end)
end
...
local function refreshESPUI()
    ...
    local espToggleFrame, espToggle, updateEspToggle = createToggle(espScrollFrame, "ESP Enabled", settings.ESP.Enabled, function(value)
        settings.ESP.Enabled = value
        updateEspToggle(value)
        updateAllESP()
    end)
    
    local distanceToggleFrame, distanceToggle, updateDistanceToggle = createToggle(espScrollFrame, "Show Distance", settings.ESP.ShowDistance, function(value)
        settings.ESP.ShowDistance = value
        updateDistanceToggle(value)
        updateAllESP()
    end)
end
