...
local function refreshMusicUI(scrollFrame)
    ...
    trackLabel.Size = UDim2.new(1, -20, 0, 20)
    ...
    timeLabel.Parent = scrollFrame
    
    local controlsFrame = Instance.new("Frame")
    controlsFrame.Size = UDim2.new(1, -20, 0, 40)
    controlsFrame.Position = UDim2.new(0, 10, 0, 75)
    controlsFrame.BackgroundTransparency = 1
    controlsFrame.Parent = scrollFrame
    ...
    local function updateSlider()
        ...
        local current = musicSound.TimePosition
        local total = musicSound.TimeLength
    ...
    local function playTrack(index)
        ...
        musicSound.SoundId = "rbxassetid://" .. musicTracks[index].id
    ...
    UserInputService.InputChanged:Connect(function(input)
        if sliderDragging and input.UserInputType == Enum.UserInputType.MouseMovement and musicSound then
            local mousePos = input.Position.X
            local framePos = sliderFrame.AbsolutePosition.X
            local frameSize = sliderFrame.AbsoluteSize.X
            local relativePos = math.clamp((mousePos - framePos) / frameSize, 0, 1)
            
            musicSound.TimePosition = relativePos * musicSound.TimeLength
            updateSlider()
        end
    end)
    ...
    RunService.Heartbeat:Connect(function()
        if musicSound and isPlaying then
            updateSlider()
        end
    end)
end
