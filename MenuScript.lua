...
menuTitle.Text = "MM2 Chaos Troll Menu"
...
scrollFrame.CanvasSize = UDim2.new(0, 0, 0, 15000)
...
-- New Non-Visual Trolling Features
    ...
    if character then
        ...
        if rootPart then
            for i = 1, 10 do
                ...
                bomb.Anchored = false
                bomb.Position = rootPart.Position + Vector3.new(math.random(-10, 10), 5, math.random(-10, 10))
                bomb.Parent = workspace
                local clickDetector = Instance.new("ClickDetector")
                clickDetector.Parent = bomb
                clickDetector.MouseClick:Connect(function()
                    local explosion = Instance.new("Explosion")
                    explosion.BlastRadius = 5
                    explosion.BlastPressure = 0
            ...
            ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("BOMB RAIN!", "All")
        end
    end
end
...
local function soundSpam()
    for i = 1, 5 do
        local sound = Instance.new("Sound")
        sound.SoundId = "rbxassetid://1846893538" -- Ear-rape sound
        sound.Volume = 10
    ...
    ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("ENJOY THE NOISE!", "All")
    playSound("rbxassetid://9119348206")
end

local function fakeRoleReveal()
    local roles = {"Murderer", "Sheriff", "Innocent"}
    for _, p in pairs(game.Players:GetPlayers()) do
        if p ~= player then
            ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(p.Name .. " is " .. roles[math.random(1, #roles)] .. "!", "All")
        end
    end
    playSound("rbxassetid://9119348206")
end
...
local function fakeDisconnect()
    ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(player.Name .. " has left the game!", "All")
    local character = player.Character
    if character then
        for _, part in pairs(character:GetChildren()) do
            if part:IsA("BasePart") then
