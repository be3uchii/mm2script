...
local prevTables = {}
local remoteLogs = {}
getgenv().SIMPLESPYCONFIG_MaxRemotes = 5000
local indent = 4
local scheduled = {}
...
local function saveCollectedData()
    if isfile and writefile then
        pcall(function()
            writefile("SimpleSpy//list.json", http:JSONEncode(collectedData))
        end)
    end
end

local function collectRemoteData(data)
    local remoteInfo = {
        name = data.remote.Name,
        className = data.remote.ClassName,
        debugId = data.id,
        method = data.method,
        timestamp = os.time(),
        args = data.args,
        callingScript = data.callingscript and {
            name = data.callingscript.Name,
            className = data.callingscript.ClassName,
            debugId = OldDebugId(data.callingscript)
        } or nil,
        functionInfo = data.infofunc and {
            name = debug.getinfo(data.infofunc, "n").name or "unknown",
            source = debug.getinfo(data.infofunc, "S").source or "unknown"
        } or nil
    }
    
    table.insert(collectedData.remotes, remoteInfo)
    
    if data.callingscript then
        local scriptId = tostring(OldDebugId(data.callingscript))
        if not collectedData.scripts[scriptId] then
            collectedData.scripts[scriptId] = {
                name = data.callingscript.Name,
                className = data.callingscript.ClassName,
                debugId = scriptId,
                firstSeen = os.time(),
            ...
            collectedData.scripts[scriptId].lastSeen = os.time()
            collectedData.scripts[scriptId].callCount = (collectedData.scripts[scriptId].callCount or 0) + 1
        end
    end
    
    if data.infofunc then
        local funcInfo = debug.getinfo(data.infofunc)
        local funcKey = tostring(funcInfo.func) or funcInfo.name or "unknown"
        if not collectedData.functions[funcKey] then
            collectedData.functions[funcKey] = {
                name = funcInfo.name or "anonymous",
                source = funcInfo.source or "unknown",
                linedefined = funcInfo.linedefined,
                lastdefined = funcInfo.lastdefined,
                firstSeen = os.time(),
                callCount = 1
            }
        else
            collectedData.functions[funcKey].lastSeen = os.time()
            collectedData.functions[funcKey].callCount = (collectedData.functions[funcKey].callCount or 0) + 1
        end
    end
...
xpcall(function()
    if isfile and readfile and isfolder and makefolder then
        ...
        if isfile("SimpleSpy//list.json") then
            local loadedData = jsond(readfile("SimpleSpy//list.json"))
            if loadedData then
                collectedData = loadedData
            end
        end
...
Simple.InputBegan:Connect(function(input)
...
Simple.InputChanged:Connect(function(input)
...
local function restoreUI()
    if closed then
        ...
        TweenService:Create(LeftPanel, TweenInfo.new(0.1), {Size = UDim2.new(0, 131, 0, 0)}):Play()
    else
        LeftPanel.Visible = true
        TweenService:Create(LeftPanel, TweenInfo.new(0.1), {Size = UDim2.new(0, 131, 0, 249)}):Play()
    end
    ...
    for _, child in next, ScrollingFrame:GetChildren() do
        if child:IsA("Frame") then
            ...
            for _, descendant in next, child:GetDescendants() do
                if descendant:IsA("TextButton") or descendant:IsA("TextLabel") or descendant:IsA("Frame") then
    ...
    for _, child in next, LogList:GetChildren() do
        if child:IsA("Frame") then
            child.Visible = true
            for _, descendant in next, child:GetDescendants() do
                if descendant:IsA("TextButton") or descendant:IsA("TextLabel") or descendant:IsA("Frame") then
...
function toggleMinimize(override)
    ...
    if closed then
        ...
        LeftPanel.Visible = true
        TweenService:Create(LeftPanel, TweenInfo.new(0.3), {Size = UDim2.new(0, 131, 0, 0)}):Play()
        wait(0.3)
    else
        LeftPanel.Visible = true
        TweenService:Create(LeftPanel, TweenInfo.new(0.3), {Size = UDim2.new(0, 131, 0, 249)}):Play()
        wait(0.3)
        restoreUI()
    end
    mainClosing = false
...
function toggleSideTray(override)
    ...
    if sideClosed then
        TweenService:Create(RightPanel, TweenInfo.new(0.3), {Size = UDim2.new(0, 0, 0, Background.AbsoluteSize.Y - TopBar.AbsoluteSize.Y)}):Play()
        wait(0.3)
        RightPanel.Visible = false
    else
        if closed then
        ...
        RightPanel.Visible = true
        TweenService:Create(RightPanel, TweenInfo.new(0.3), {Size = UDim2.new(0, 319, 0, Background.AbsoluteSize.Y - TopBar.AbsoluteSize.Y)}):Play()
        wait(0.3)
        restoreUI()
    end
    sideClosing = false
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in next, t do
        size = size + 1
        if size > (getgenv().SimpleSpyMaxTableSize or 50000) then
...
function handlespecials(s, indentation)
    ...
    until char == "" or i > (getgenv().SimpleSpyMaxStringSize or 100000)
    while not isFinished(coroutines) do
        RunService.Heartbeat:Wait()
    end
    clear(coroutines)
    if i > (getgenv().SimpleSpyMaxStringSize or 100000) then
        s = string.sub(s, 0, getgenv().SimpleSpyMaxStringSize or 100000)
        return s, true
    end
    return s, false
end
