...
local function logAllCalls(method, ...)
    if not configs.logallcalls then
        return
    end
    
    local args = {...}
    local callingScript = getcallingscript()
    local funcInfo = info(2, "f")
    
    if callingScript and funcInfo then
        local data = {
            method = method,
            args = deepclone(args),
        ...
        schedule(function()
            if not table.find(blacklist, callingScript.Name) then
                newRemote("function", {
                    remote = callingScript,
                    callingscript = callingScript,
                    args = args,
                    infofunc = funcInfo,
                    id = ThreadGetDebugId(callingScript),
                    metamethod = "call"
                })
            end
        end)
...
function remoteHandler(data)
    ...
    elseif configs.logallcalls then
        newRemote("function", data)
    end
end
...
newButton(
    "Log All Calls",
    function() return string.format("[%s] Log absolutely all function calls and methods", configs.logallcalls and "ENABLED" or "DISABLED") end,
    function()
        configs.logallcalls = not configs.logallcalls
        TextLabel.Text = string.format("[%s] Log absolutely all function calls and methods", configs.logallcalls and "ENABLED" or "DISABLED")
    end
)