...
local prevTables = {}
local remoteLogs = {}
getgenv().SIMPLESPYCONFIG_MaxRemotes = 10000
local indent = 4
local scheduled = {}
...
function toggleSideTray(override)
    ...
    if sideClosed then
        TweenService:Create(RightPanel, TweenInfo.new(0.5), {Size = UDim2.new(0, 0, 0, Background.AbsoluteSize.Y - TopBar.AbsoluteSize.Y)}):Play()
        wait(0.5)
        RightPanel.Visible = false
    else
        RightPanel.Visible = true
        TweenService:Create(RightPanel, TweenInfo.new(0.5), {Size = UDim2.new(0, 319, 0, Background.AbsoluteSize.Y - TopBar.AbsoluteSize.Y)}):Play()
        wait(0.5)
        bringBackOnResize()
    end
    sideClosing = false
