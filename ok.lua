...
local prevTables = {}
local remoteLogs = {}
getgenv().SIMPLESPYCONFIG_MaxRemotes = 2000
local indent = 4
local scheduled = {}
...
local dragStartPos
local startBackgroundPos

local function startDrag()
    dragging = true
    dragStartPos = UserInputService:GetMouseLocation()
    startBackgroundPos = Vector2.new(Background.Position.X.Offset, Background.Position.Y.Offset)
end

local function endDrag()
    dragging = false
    dragStartPos = nil
    startBackgroundPos = nil
end
...
local function updateDrag()
    if dragging then
        ...
        local newPos = startBackgroundPos + delta
        
        -- Ограничение перемещения в пределах экрана
        local viewportSize = workspace.CurrentCamera.ViewportSize
        newPos = Vector2.new(
            math.clamp(newPos.X, 0, viewportSize.X - Background.AbsoluteSize.X),
            math.clamp(newPos.Y, 0, viewportSize.Y - Background.AbsoluteSize.Y - GuiInset.Y)
        )
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in next, t do
        size = size + 1
        if size > (getgenv().SimpleSpyMaxTableSize or 10000) then
            s = s .. "\n" .. string.rep(" ", l) .. "-- LARGE TABLE - " .. size .. " ELEMENTS"
            break
        end
...
function formatstr(s, indentation)
    ...
    return '"' .. handled .. '"' .. (reachedMax and " --[[LARGE STRING - " .. #s .. " CHARACTERS]]" or "")
end
...
function handlespecials(s, indentation)
    ...
    until char == "" or i > (getgenv().SimpleSpyMaxStringSize or 50000)
    while not isFinished(coroutines) do
        RunService.Heartbeat:Wait()
    end
    clear(coroutines)
    if i > (getgenv().SimpleSpyMaxStringSize or 50000) then
        s = string.sub(s, 0, getgenv().SimpleSpyMaxStringSize or 50000)
        return s, true
    end
    return s, false
end
