...
function handlespecials(s, indentation)
    ...
    until char == "" or i > (getgenv().SimpleSpyMaxStringSize or 30000)
    while not isFinished(coroutines) do
        RunService.Heartbeat:Wait()
    end
    clear(coroutines)
    if i > (getgenv().SimpleSpyMaxStringSize or 30000) then
        s = string.sub(s, 0, getgenv().SimpleSpyMaxStringSize or 30000)
        return s, true
    end
    return s, false
end
