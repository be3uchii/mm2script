...
local function isPlayerVisible(player)
    ...
    local ray = Ray.new(origin, direction * (humanoidRootPart.Position - origin).Magnitude)
    
    local hitPart, hitPosition = workspace:FindPartOnRayWithIgnoreList(ray, {LocalPlayer.Character, player.Character})
    
    return not hitPart or hitPart:IsDescendantOf(player.Character)
end
...
local function updateHitboxes()
    ...
    local function applyHitbox(player)
        if player ~= LocalPlayer and player.Character then
            ...
            local humanoidRootPart = player.Character:FindFirstChild("HumanoidRootPart")
            if humanoid and humanoidRootPart and humanoid.Health > 0 and isPlayerVisible(player) then
