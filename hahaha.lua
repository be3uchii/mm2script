...
local hitboxUpdateCooldown = 0.3
...
local function updateESP()
    ...
    local currentTime = tick()
    if currentTime - lastESPUpdate < espUpdateCooldown then return end
    ...
    if not connections.ESP then
        connections.ESP = RunService.Heartbeat:Connect(function()
            if tick() - lastESPUpdate < espUpdateCooldown then return end
            lastESPUpdate = tick()
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character then
                    local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
...
for i, tabName in ipairs(tabs) do
    ...
    if tabName == "Combat" then
        ...
        createValueChanger(scrollFrame, "Размер Хитбокса", {1,2,3,4,5}, settings.Combat.Hitbox.Size, function(value)
