...
local function createPasswordGUI()
    ...
    confirmButton.MouseButton1Click:Connect(function()
        playSound()
        if passwordBox.Text == "1234ok" then
            ensureConfigFolder()
            writefile(configFolder.."/"..keyFile, "authenticated")
            screenGui:Destroy()
            createMainGUI()
        else
            passwordBox.Text = ""
...
local function checkKeyFile()
    ensureConfigFolder()
    return isfile(configFolder.."/"..keyFile)
end

if not checkKeyFile() then
    createPasswordGUI()
else
    createMainGUI()
end
