...
local keyFile = "silence_key_" .. LocalPlayer.UserId .. ".txt"
...
local function checkKeyFile()
    ensureConfigFolder()
    if not isfile(configFolder.."/"..keyFile) then
        return false
    end
    local success, result = pcall(function()
        return HttpService:JSONDecode(readfile(configFolder.."/"..keyFile))
    end)
    if success and result and result.password and result.timestamp then
        local currentTime = os.time()
        local keyAge = currentTime - result.timestamp
        if keyAge < 24 * 3600 then
            return true, result.password
        else
            delfile(configFolder.."/"..keyFile)
            return false
        end
    end
    return false
...
local function saveKeyFile(password)
    ensureConfigFolder()
    local data = {
        password = password,
        timestamp = os.time()
    }
    writefile(configFolder.."/"..keyFile, HttpService:JSONEncode(data))
end
...
local function createPasswordGUI()
    ...
    timerLabel.TextColor3 = Color3.fromRGB(230, 230, 230)
    timerLabel.TextTransparency = 0.1
    timerLabel.TextSize = 12
    timerLabel.Font = Enum.Font.GothamBold
    timerLabel.TextXAlignment = Enum.TextXAlignment.Center
    ...
    generateButton.Size = UDim2.new(0.38, 0, 0, 30)
    generateButton.Position = UDim2.new(0.1, 0, 0, 90)
    ...
    generateButton.TextSize = 10
    generateButton.Font = Enum.Font.GothamBold
    generateButton.Parent = mainFrame

    local confirmButton = Instance.new("TextButton")
    confirmButton.Size = UDim2.new(0.38, 0, 0, 30)
    confirmButton.Position = UDim2.new(0.52, 0, 0, 90)
    ...
    corner:Clone().Parent = confirmButton

    local timerStart = os.time()
    local timerDuration = 60
    local canGenerate = false
    local passwordGenerated = false

    local function updateTimerLabel()
        local elapsed = os.time() - timerStart
        local remaining = math.max(0, timerDuration - elapsed)
        timerLabel.Text = string.format("00:%02d", remaining)
        if remaining == 0 and not passwordGenerated then
            canGenerate = true
            generateButton.BackgroundColor3 = Color3.fromRGB(76, 217, 100)
            local newPassword = generatePassword()
            passwordBox.Text = newPassword
            passwordGenerated = true
            generateButton.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
    ...
    generateButton.MouseButton1Click:Connect(function()
        if canGenerate and not passwordGenerated then
            playSound()
            local newPassword = generatePassword()
            passwordBox.Text = newPassword
            passwordGenerated = true
    ...
    confirmButton.MouseButton1Click:Connect(function()
        playSound()
        local isValid, storedPassword = checkKeyFile()
        if isValid and passwordBox.Text == storedPassword or passwordGenerated then
            if passwordGenerated then
                saveKeyFile(passwordBox.Text)
            end
            ...
            for _, child in ipairs(mainFrame:GetDescendants()) do
                if child:IsA("GuiObject") then
                    TweenService:Create(child, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1, TextTransparency = 1, ImageTransparency = 1}):Play()
                end
            end
            ...
            passwordBox.PlaceholderColor3 = Color3.fromRGB(255, 58, 58)
            task.wait(1)
            passwordBox.PlaceholderText = "Пароль"
...
local function checkAndShowGUI()
    local isValid = checkKeyFile()
    if isValid then
        createMainGUI()
    else
        createPasswordGUI()
...
game:BindToClose(function()
    ...
    if PlayerGui:FindFirstChild("PasswordGui") then
        PlayerGui.PasswordGui:Destroy()
    end
